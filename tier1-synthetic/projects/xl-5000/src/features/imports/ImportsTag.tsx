...
import styles from './ImportsTag.module.css';

interface ImportsTagItem {
  title: string;
  status: string;
  priority: number;
  updatedAt: Date;
...
interface ImportsTagProps {
  items?: ImportsTagItem[];
...
export default function ImportsTag({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'status', 'priority']);
