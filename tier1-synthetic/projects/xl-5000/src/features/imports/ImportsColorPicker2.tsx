...
import styles from './ImportsColorPicker2.module.css';

const ImportsColorPicker2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ImportsColorPicker2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ImportsColorPicker2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ImportsColorPicker2Schema.safeParse({ value: val });
