...
import ImportsRating from './ImportsRating';
import styles from './ImportsDetail.module.css';

interface ImportsDetailItem {
  createdAt: Date;
  status: string;
  title: string;
  assignee: string;
...
interface ImportsDetailProps {
  items?: ImportsDetailItem[];
...
export default function ImportsDetail({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'status', 'title']);
