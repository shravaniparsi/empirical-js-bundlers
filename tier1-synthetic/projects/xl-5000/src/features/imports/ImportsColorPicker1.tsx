...
import { z } from 'zod';
import ImportsDonut1 from './ImportsDonut1';
import styles from './ImportsColorPicker1.module.css';

const ImportsColorPicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ImportsColorPicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ImportsColorPicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ImportsColorPicker1Schema.safeParse({ value: val });
