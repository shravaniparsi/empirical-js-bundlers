...
import { z } from 'zod';
import ImportsAdapter1 from './ImportsAdapter1';
import styles from './ImportsTimePicker.module.css';

const ImportsTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ImportsTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ImportsTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ImportsTimePickerSchema.safeParse({ value: val });
