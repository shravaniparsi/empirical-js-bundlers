...
import TemplatesThumbnail1 from './TemplatesThumbnail1';
import styles from './TemplatesSummary2.module.css';

interface TemplatesSummary2Item {
  title: string;
  createdAt: Date;
  priority: number;
  tags: string[];
...
interface TemplatesSummary2Props {
  items?: TemplatesSummary2Item[];
...
export default function TemplatesSummary2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: TemplatesSummary2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'createdAt', 'priority']);
