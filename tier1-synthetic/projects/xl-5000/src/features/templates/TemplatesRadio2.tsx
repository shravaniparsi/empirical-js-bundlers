...
import styles from './TemplatesRadio2.module.css';

const TemplatesRadio2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TemplatesRadio2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TemplatesRadio2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TemplatesRadio2Schema.safeParse({ value: val });
