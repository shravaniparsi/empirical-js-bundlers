...
import { z } from 'zod';
import TemplatesLoader1 from './TemplatesLoader1';
import TemplatesList from './TemplatesList';
import RolesProgress1 from '../roles/RolesProgress1';
import styles from './TemplatesCheckbox2.module.css';

const TemplatesCheckbox2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TemplatesCheckbox2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TemplatesCheckbox2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TemplatesCheckbox2Schema.safeParse({ value: val });
