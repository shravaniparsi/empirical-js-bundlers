...
import styles from './TemplatesMeter.module.css';

interface TemplatesMeterItem {
  priority: number;
  tags: string[];
  status: string;
  id: string;
...
interface TemplatesMeterProps {
  items?: TemplatesMeterItem[];
...
export default function TemplatesMeter({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['priority', 'tags', 'status']);
