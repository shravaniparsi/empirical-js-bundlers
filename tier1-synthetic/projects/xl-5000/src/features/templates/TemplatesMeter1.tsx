...
import TemplatesNotification from './TemplatesNotification';
import TemplatesBarChart from './TemplatesBarChart';
import styles from './TemplatesMeter1.module.css';

interface TemplatesMeter1Item {
  createdAt: Date;
  assignee: string;
  updatedAt: Date;
  tags: string[];
...
interface TemplatesMeter1Props {
  items?: TemplatesMeter1Item[];
...
export default function TemplatesMeter1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: TemplatesMeter1Props) {
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [sortField, setSortField] = useState<'createdAt' | 'assignee'>('createdAt');
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'assignee', 'updatedAt']);
