...
import ReviewsToggle from '../reviews/ReviewsToggle';
import ApiKeysColorPicker1 from '../api-keys/ApiKeysColorPicker1';
import styles from './TemplatesColorPicker.module.css';

const TemplatesColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TemplatesColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TemplatesColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TemplatesColorPickerSchema.safeParse({ value: val });
