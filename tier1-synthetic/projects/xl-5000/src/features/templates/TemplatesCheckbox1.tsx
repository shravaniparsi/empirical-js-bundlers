...
import styles from './TemplatesCheckbox1.module.css';

const TemplatesCheckbox1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TemplatesCheckbox1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TemplatesCheckbox1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TemplatesCheckbox1Schema.safeParse({ value: val });
