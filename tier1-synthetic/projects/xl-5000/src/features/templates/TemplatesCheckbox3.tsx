...
import styles from './TemplatesCheckbox3.module.css';

const TemplatesCheckbox3Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TemplatesCheckbox3Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TemplatesCheckbox3({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TemplatesCheckbox3Schema.safeParse({ value: val });
