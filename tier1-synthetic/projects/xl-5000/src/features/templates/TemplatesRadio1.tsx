...
import { z } from 'zod';
import TemplatesRetry from './TemplatesRetry';
import styles from './TemplatesRadio1.module.css';

const TemplatesRadio1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TemplatesRadio1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TemplatesRadio1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TemplatesRadio1Schema.safeParse({ value: val });
