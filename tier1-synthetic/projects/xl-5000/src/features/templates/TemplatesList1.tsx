...
import TemplatesHeatmap from './TemplatesHeatmap';
import TemplatesPagination from './TemplatesPagination';
import TemplatesScore from './TemplatesScore';
import WorkflowsSuspense from '../workflows/WorkflowsSuspense';
import styles from './TemplatesList1.module.css';

interface TemplatesList1Item {
  assignee: string;
  tags: string[];
  createdAt: Date;
  priority: number;
...
interface TemplatesList1Props {
  items?: TemplatesList1Item[];
...
export default function TemplatesList1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'tags', 'createdAt']);
