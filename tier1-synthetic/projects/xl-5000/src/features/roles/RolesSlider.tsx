...
import RolesThumbnail1 from './RolesThumbnail1';
import styles from './RolesSlider.module.css';

const RolesSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface RolesSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function RolesSlider({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = RolesSliderSchema.safeParse({ value: val });
