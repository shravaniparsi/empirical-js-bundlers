...
import { z } from 'zod';
import RolesGauge1 from './RolesGauge1';
import RolesTable1 from './RolesTable1';
import styles from './RolesRating1.module.css';

const RolesRating1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface RolesRating1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function RolesRating1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = RolesRating1Schema.safeParse({ value: val });
