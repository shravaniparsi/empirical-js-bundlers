...
import { z } from 'zod';
import RolesSticky from './RolesSticky';
...
import styles from './RolesRangeSlider.module.css';

const RolesRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface RolesRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function RolesRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = RolesRangeSliderSchema.safeParse({ value: val });
