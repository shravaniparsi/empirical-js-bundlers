...
import { z } from 'zod';
import RolesSummary1 from './RolesSummary1';
import styles from './RolesCheckbox3.module.css';

const RolesCheckbox3Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface RolesCheckbox3Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function RolesCheckbox3({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = RolesCheckbox3Schema.safeParse({ value: val });
