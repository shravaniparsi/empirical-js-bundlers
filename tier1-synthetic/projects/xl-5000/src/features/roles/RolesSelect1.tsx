...
import { z } from 'zod';
import RolesRadio1 from './RolesRadio1';
import RolesFooter2 from './RolesFooter2';
import styles from './RolesSelect1.module.css';

const RolesSelect1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface RolesSelect1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function RolesSelect1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = RolesSelect1Schema.safeParse({ value: val });
