...
import { z } from 'zod';
import RolesSearch2 from './RolesSearch2';
import RolesProgress from './RolesProgress';
import RolesSpacer from './RolesSpacer';
import styles from './RolesCheckbox4.module.css';

const RolesCheckbox4Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface RolesCheckbox4Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function RolesCheckbox4({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = RolesCheckbox4Schema.safeParse({ value: val });
