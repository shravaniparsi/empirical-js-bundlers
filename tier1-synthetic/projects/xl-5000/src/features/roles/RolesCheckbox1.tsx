...
import RolesPolling from './RolesPolling';
import RolesPagination1 from './RolesPagination1';
import styles from './RolesCheckbox1.module.css';

const RolesCheckbox1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface RolesCheckbox1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function RolesCheckbox1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = RolesCheckbox1Schema.safeParse({ value: val });
