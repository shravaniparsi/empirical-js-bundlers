...
import RolesColorPicker from './RolesColorPicker';
import CheckoutRating from '../checkout/CheckoutRating';
import styles from './RolesAvatar2.module.css';

interface RolesAvatar2Item {
  assignee: string;
  priority: number;
  tags: string[];
  title: string;
...
interface RolesAvatar2Props {
  items?: RolesAvatar2Item[];
...
export default function RolesAvatar2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'priority', 'tags']);
