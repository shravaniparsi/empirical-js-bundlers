...
import styles from './CommentsTimePicker.module.css';

const CommentsTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CommentsTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CommentsTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CommentsTimePickerSchema.safeParse({ value: val });
