...
import CommentsSplit1 from './CommentsSplit1';
import ReviewsErrorBoundary from '../reviews/ReviewsErrorBoundary';
import styles from './CommentsChip2.module.css';

interface CommentsChip2Item {
  createdAt: Date;
  priority: number;
  updatedAt: Date;
  title: string;
...
interface CommentsChip2Props {
  items?: CommentsChip2Item[];
...
export default function CommentsChip2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'priority', 'updatedAt']);
