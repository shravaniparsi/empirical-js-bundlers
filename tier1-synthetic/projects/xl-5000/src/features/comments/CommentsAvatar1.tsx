...
import CommentsSidebar1 from './CommentsSidebar1';
import styles from './CommentsAvatar1.module.css';

interface CommentsAvatar1Item {
  assignee: string;
  createdAt: Date;
  title: string;
  priority: number;
...
interface CommentsAvatar1Props {
  items?: CommentsAvatar1Item[];
...
export default function CommentsAvatar1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: CommentsAvatar1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'createdAt', 'title']);
