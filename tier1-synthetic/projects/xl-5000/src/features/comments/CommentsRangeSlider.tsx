...
import { z } from 'zod';
import CommentsChip1 from './CommentsChip1';
import CommentsLabel from './CommentsLabel';
import CommentsTag from './CommentsTag';
import styles from './CommentsRangeSlider.module.css';

const CommentsRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CommentsRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CommentsRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CommentsRangeSliderSchema.safeParse({ value: val });
