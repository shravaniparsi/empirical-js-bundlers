...
import styles from './IntegrationsAvatar.module.css';

interface IntegrationsAvatarItem {
  tags: string[];
  status: string;
  description: string;
  title: string;
...
interface IntegrationsAvatarProps {
  items?: IntegrationsAvatarItem[];
...
export default function IntegrationsAvatar({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: IntegrationsAvatarProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'status', 'description']);
