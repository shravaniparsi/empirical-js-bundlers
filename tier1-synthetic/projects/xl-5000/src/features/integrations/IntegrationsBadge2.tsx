...
import IntegrationsErrorBoundary from './IntegrationsErrorBoundary';
import IntegrationsNotification from './IntegrationsNotification';
import styles from './IntegrationsBadge2.module.css';

interface IntegrationsBadge2Item {
  priority: number;
  status: string;
  description: string;
  createdAt: Date;
...
interface IntegrationsBadge2Props {
  items?: IntegrationsBadge2Item[];
...
export default function IntegrationsBadge2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: IntegrationsBadge2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['priority', 'status', 'description']);
