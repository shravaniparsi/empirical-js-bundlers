...
import IntegrationsRank from './IntegrationsRank';
import styles from './IntegrationsThumbnail1.module.css';

interface IntegrationsThumbnail1Item {
  tags: string[];
  priority: number;
  status: string;
  id: string;
...
interface IntegrationsThumbnail1Props {
  items?: IntegrationsThumbnail1Item[];
...
export default function IntegrationsThumbnail1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: IntegrationsThumbnail1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'priority', 'status']);
