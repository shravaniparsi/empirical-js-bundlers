...
import styles from './IntegrationsAutocomplete.module.css';

const IntegrationsAutocompleteSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface IntegrationsAutocompleteProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function IntegrationsAutocomplete({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = IntegrationsAutocompleteSchema.safeParse({ value: val });
