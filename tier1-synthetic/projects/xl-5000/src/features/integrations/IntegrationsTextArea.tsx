...
import { z } from 'zod';
import IntegrationsThumbnail1 from './IntegrationsThumbnail1';
import styles from './IntegrationsTextArea.module.css';

const IntegrationsTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface IntegrationsTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function IntegrationsTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = IntegrationsTextAreaSchema.safeParse({ value: val });
