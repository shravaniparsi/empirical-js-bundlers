...
import IntegrationsAreaChart from './IntegrationsAreaChart';
import styles from './IntegrationsFeed.module.css';

interface IntegrationsFeedItem {
  title: string;
  createdAt: Date;
  assignee: string;
  status: string;
...
interface IntegrationsFeedProps {
  items?: IntegrationsFeedItem[];
...
export default function IntegrationsFeed({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: IntegrationsFeedProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'createdAt', 'assignee']);
