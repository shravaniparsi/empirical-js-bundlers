...
import IntegrationsBreadcrumb from './IntegrationsBreadcrumb';
import IntegrationsSnackbar2 from './IntegrationsSnackbar2';
import styles from './IntegrationsStat.module.css';

interface IntegrationsStatItem {
  updatedAt: Date;
  description: string;
  createdAt: Date;
  priority: number;
...
interface IntegrationsStatProps {
  items?: IntegrationsStatItem[];
...
export default function IntegrationsStat({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: IntegrationsStatProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'description', 'createdAt']);
