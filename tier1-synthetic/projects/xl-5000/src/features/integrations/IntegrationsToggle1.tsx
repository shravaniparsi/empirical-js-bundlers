...
import { z } from 'zod';
import IntegrationsPolling from './IntegrationsPolling';
import styles from './IntegrationsToggle1.module.css';

const IntegrationsToggle1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface IntegrationsToggle1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function IntegrationsToggle1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = IntegrationsToggle1Schema.safeParse({ value: val });
