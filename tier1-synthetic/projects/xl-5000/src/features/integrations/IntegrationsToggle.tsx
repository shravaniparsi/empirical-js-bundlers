...
import { z } from 'zod';
import IntegrationsRangeSlider from './IntegrationsRangeSlider';
import styles from './IntegrationsToggle.module.css';

const IntegrationsToggleSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface IntegrationsToggleProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function IntegrationsToggle({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = IntegrationsToggleSchema.safeParse({ value: val });
