...
import IntegrationsBarChart1 from './IntegrationsBarChart1';
...
import styles from './IntegrationsLabel.module.css';

interface IntegrationsLabelItem {
  assignee: string;
  createdAt: Date;
  status: string;
  id: string;
...
interface IntegrationsLabelProps {
  items?: IntegrationsLabelItem[];
...
export default function IntegrationsLabel({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: IntegrationsLabelProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'createdAt', 'status']);
