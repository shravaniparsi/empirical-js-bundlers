...
import styles from './BillingCheckbox3.module.css';

const BillingCheckbox3Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface BillingCheckbox3Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function BillingCheckbox3({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = BillingCheckbox3Schema.safeParse({ value: val });
