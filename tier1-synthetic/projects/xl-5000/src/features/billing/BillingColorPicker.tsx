...
import styles from './BillingColorPicker.module.css';

const BillingColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface BillingColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function BillingColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = BillingColorPickerSchema.safeParse({ value: val });
