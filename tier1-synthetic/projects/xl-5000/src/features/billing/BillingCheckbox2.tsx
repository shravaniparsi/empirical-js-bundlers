...
import { z } from 'zod';
import BillingTabs1 from './BillingTabs1';
import styles from './BillingCheckbox2.module.css';

const BillingCheckbox2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface BillingCheckbox2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function BillingCheckbox2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = BillingCheckbox2Schema.safeParse({ value: val });
