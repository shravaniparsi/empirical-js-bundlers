...
import { z } from 'zod';
import BillingPreview1 from './BillingPreview1';
import BillingDetail2 from './BillingDetail2';
import BillingInfiniteScroll4 from './BillingInfiniteScroll4';
import styles from './BillingTextArea.module.css';

const BillingTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface BillingTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function BillingTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = BillingTextAreaSchema.safeParse({ value: val });
