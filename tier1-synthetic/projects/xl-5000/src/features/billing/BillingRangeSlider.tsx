...
import styles from './BillingRangeSlider.module.css';

const BillingRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface BillingRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function BillingRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = BillingRangeSliderSchema.safeParse({ value: val });
