...
import { z } from 'zod';
import SearchGrid2 from './SearchGrid2';
import styles from './SearchAutocomplete3.module.css';

const SearchAutocomplete3Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SearchAutocomplete3Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SearchAutocomplete3({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SearchAutocomplete3Schema.safeParse({ value: val });
