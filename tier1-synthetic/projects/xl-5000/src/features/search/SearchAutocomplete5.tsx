...
import styles from './SearchAutocomplete5.module.css';

const SearchAutocomplete5Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SearchAutocomplete5Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SearchAutocomplete5({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SearchAutocomplete5Schema.safeParse({ value: val });
