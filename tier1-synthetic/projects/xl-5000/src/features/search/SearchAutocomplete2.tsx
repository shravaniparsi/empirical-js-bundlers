...
import styles from './SearchAutocomplete2.module.css';

const SearchAutocomplete2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SearchAutocomplete2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SearchAutocomplete2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SearchAutocomplete2Schema.safeParse({ value: val });
