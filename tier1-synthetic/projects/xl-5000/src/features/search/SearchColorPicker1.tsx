...
import RolesFooter1 from '../roles/RolesFooter1';
import styles from './SearchColorPicker1.module.css';

const SearchColorPicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SearchColorPicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SearchColorPicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SearchColorPicker1Schema.safeParse({ value: val });
