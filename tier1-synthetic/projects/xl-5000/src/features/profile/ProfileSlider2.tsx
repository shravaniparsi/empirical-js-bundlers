...
import ProfileTooltip1 from './ProfileTooltip1';
...
import styles from './ProfileSlider2.module.css';

const ProfileSlider2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProfileSlider2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProfileSlider2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProfileSlider2Schema.safeParse({ value: val });
