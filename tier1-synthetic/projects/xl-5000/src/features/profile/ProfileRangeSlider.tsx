...
import { z } from 'zod';
import ProfileStepper1 from './ProfileStepper1';
import styles from './ProfileRangeSlider.module.css';

const ProfileRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProfileRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProfileRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProfileRangeSliderSchema.safeParse({ value: val });
