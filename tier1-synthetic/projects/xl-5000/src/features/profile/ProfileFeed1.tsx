...
import AuditSplit from '../audit/AuditSplit';
import InventoryChip3 from '../inventory/InventoryChip3';
import styles from './ProfileFeed1.module.css';

interface ProfileFeed1Item {
  createdAt: Date;
  priority: number;
  tags: string[];
  updatedAt: Date;
...
interface ProfileFeed1Props {
  items?: ProfileFeed1Item[];
...
export default function ProfileFeed1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'priority', 'tags']);
