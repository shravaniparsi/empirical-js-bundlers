...
import ProfileAreaChart from './ProfileAreaChart';
import ProfileStepper2 from './ProfileStepper2';
import LocalizationBreadcrumb from '../localization/LocalizationBreadcrumb';
import ExportsDivider2 from '../exports/ExportsDivider2';
import ApiKeysRating from '../api-keys/ApiKeysRating';
import FeedbackMgmtDrawer1 from '../feedback-mgmt/FeedbackMgmtDrawer1';
import styles from './ProfileScore2.module.css';

interface ProfileScore2Item {
  updatedAt: Date;
  assignee: string;
  tags: string[];
  createdAt: Date;
...
interface ProfileScore2Props {
  items?: ProfileScore2Item[];
...
export default function ProfileScore2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  ...
  const [sortField, setSortField] = useState<'updatedAt' | 'assignee'>('updatedAt');
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'assignee', 'tags']);
