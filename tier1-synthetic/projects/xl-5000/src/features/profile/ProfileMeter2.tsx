...
import ProfileNavBar1 from './ProfileNavBar1';
import SearchGauge from '../search/SearchGauge';
import styles from './ProfileMeter2.module.css';

interface ProfileMeter2Item {
  description: string;
  status: string;
  assignee: string;
  tags: string[];
...
interface ProfileMeter2Props {
  items?: ProfileMeter2Item[];
...
export default function ProfileMeter2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'status', 'assignee']);
