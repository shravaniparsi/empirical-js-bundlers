...
import styles from './ProfileSlider3.module.css';

const ProfileSlider3Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProfileSlider3Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProfileSlider3({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProfileSlider3Schema.safeParse({ value: val });
