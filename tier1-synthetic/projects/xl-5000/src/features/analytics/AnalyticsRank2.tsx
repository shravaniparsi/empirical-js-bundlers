...
import AnalyticsProgress3 from './AnalyticsProgress3';
import styles from './AnalyticsRank2.module.css';

interface AnalyticsRank2Item {
  assignee: string;
  createdAt: Date;
  updatedAt: Date;
  title: string;
...
interface AnalyticsRank2Props {
  items?: AnalyticsRank2Item[];
...
export default function AnalyticsRank2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'createdAt', 'updatedAt']);
