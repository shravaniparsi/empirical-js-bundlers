...
import AnalyticsFunnel1 from './AnalyticsFunnel1';
import AnalyticsToggle from './AnalyticsToggle';
import styles from './AnalyticsSummary3.module.css';

interface AnalyticsSummary3Item {
  title: string;
  updatedAt: Date;
  status: string;
  tags: string[];
...
interface AnalyticsSummary3Props {
  items?: AnalyticsSummary3Item[];
...
export default function AnalyticsSummary3({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: AnalyticsSummary3Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'updatedAt', 'status']);
