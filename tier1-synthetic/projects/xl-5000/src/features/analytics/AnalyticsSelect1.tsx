...
import { z } from 'zod';
import AnalyticsSparkline4 from './AnalyticsSparkline4';
import ReportsMeter1 from '../reports/ReportsMeter1';
import styles from './AnalyticsSelect1.module.css';

const AnalyticsSelect1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AnalyticsSelect1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AnalyticsSelect1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AnalyticsSelect1Schema.safeParse({ value: val });
