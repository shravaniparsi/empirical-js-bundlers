...
import { z } from 'zod';
import AnalyticsResponsive from './AnalyticsResponsive';
import AnalyticsList1 from './AnalyticsList1';
import AnalyticsHeader from './AnalyticsHeader';
import FeedbackMgmtDrawer1 from '../feedback-mgmt/FeedbackMgmtDrawer1';
import styles from './AnalyticsFileUpload.module.css';

const AnalyticsFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AnalyticsFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AnalyticsFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AnalyticsFileUploadSchema.safeParse({ value: val });
