...
import styles from './AnalyticsProgress3.module.css';

interface AnalyticsProgress3Item {
  updatedAt: Date;
  priority: number;
  description: string;
  status: string;
...
interface AnalyticsProgress3Props {
  items?: AnalyticsProgress3Item[];
...
export default function AnalyticsProgress3({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: AnalyticsProgress3Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'priority', 'description']);
