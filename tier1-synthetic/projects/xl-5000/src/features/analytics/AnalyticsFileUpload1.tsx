...
import styles from './AnalyticsFileUpload1.module.css';

const AnalyticsFileUpload1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AnalyticsFileUpload1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AnalyticsFileUpload1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AnalyticsFileUpload1Schema.safeParse({ value: val });
