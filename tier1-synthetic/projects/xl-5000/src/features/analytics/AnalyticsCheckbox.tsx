...
import { z } from 'zod';
import AnalyticsLabel1 from './AnalyticsLabel1';
import AnalyticsPrefetch from './AnalyticsPrefetch';
import AnalyticsProgress2 from './AnalyticsProgress2';
import AuthList from '../auth/AuthList';
import styles from './AnalyticsCheckbox.module.css';

const AnalyticsCheckboxSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AnalyticsCheckboxProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AnalyticsCheckbox({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AnalyticsCheckboxSchema.safeParse({ value: val });
