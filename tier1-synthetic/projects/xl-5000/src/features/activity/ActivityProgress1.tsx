...
import styles from './ActivityProgress1.module.css';

interface ActivityProgress1Item {
  assignee: string;
  description: string;
  createdAt: Date;
  status: string;
...
interface ActivityProgress1Props {
  items?: ActivityProgress1Item[];
...
export default function ActivityProgress1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: ActivityProgress1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'description', 'createdAt']);
