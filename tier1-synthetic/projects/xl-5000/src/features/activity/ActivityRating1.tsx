...
import styles from './ActivityRating1.module.css';

const ActivityRating1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ActivityRating1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ActivityRating1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ActivityRating1Schema.safeParse({ value: val });
