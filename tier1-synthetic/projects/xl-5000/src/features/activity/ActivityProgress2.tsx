...
import CartConfirm1 from '../cart/CartConfirm1';
import DocumentsContext from '../documents/DocumentsContext';
import styles from './ActivityProgress2.module.css';

interface ActivityProgress2Item {
  updatedAt: Date;
  title: string;
  createdAt: Date;
  status: string;
...
interface ActivityProgress2Props {
  items?: ActivityProgress2Item[];
...
export default function ActivityProgress2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: ActivityProgress2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'title', 'createdAt']);
