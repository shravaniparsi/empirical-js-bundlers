...
import styles from './ActivitySearch3.module.css';

const ActivitySearch3Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ActivitySearch3Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ActivitySearch3({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ActivitySearch3Schema.safeParse({ value: val });
