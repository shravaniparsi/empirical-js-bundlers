...
import ExportsChip2 from '../exports/ExportsChip2';
import IntegrationsScroll1 from '../integrations/IntegrationsScroll1';
import styles from './ActivityGrid2.module.css';

interface ActivityGrid2Item {
  assignee: string;
  title: string;
  updatedAt: Date;
  description: string;
...
interface ActivityGrid2Props {
  items?: ActivityGrid2Item[];
...
export default function ActivityGrid2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'title', 'updatedAt']);
