...
import styles from './ActivitySearch1.module.css';

const ActivitySearch1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ActivitySearch1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ActivitySearch1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ActivitySearch1Schema.safeParse({ value: val });
