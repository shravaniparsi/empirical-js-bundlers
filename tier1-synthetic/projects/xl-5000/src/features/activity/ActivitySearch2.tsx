...
import styles from './ActivitySearch2.module.css';

const ActivitySearch2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ActivitySearch2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ActivitySearch2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ActivitySearch2Schema.safeParse({ value: val });
