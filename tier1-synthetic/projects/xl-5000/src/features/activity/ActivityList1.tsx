...
import styles from './ActivityList1.module.css';

interface ActivityList1Item {
  title: string;
  assignee: string;
  tags: string[];
  createdAt: Date;
...
interface ActivityList1Props {
  items?: ActivityList1Item[];
...
export default function ActivityList1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'assignee', 'tags']);
