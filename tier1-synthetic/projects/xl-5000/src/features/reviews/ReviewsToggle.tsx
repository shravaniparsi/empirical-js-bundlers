...
import { z } from 'zod';
import CategoriesInfiniteScroll from '../categories/CategoriesInfiniteScroll';
import KnowledgeBaseFunnel1 from '../knowledge-base/KnowledgeBaseFunnel1';
import styles from './ReviewsToggle.module.css';

const ReviewsToggleSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsToggleProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsToggle({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsToggleSchema.safeParse({ value: val });
