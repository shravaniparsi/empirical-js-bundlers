...
import styles from './ReviewsSearch1.module.css';

const ReviewsSearch1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsSearch1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsSearch1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsSearch1Schema.safeParse({ value: val });
