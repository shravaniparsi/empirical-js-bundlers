...
import ReviewsStat from './ReviewsStat';
import styles from './ReviewsColorPicker.module.css';

const ReviewsColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsColorPickerSchema.safeParse({ value: val });
