...
import styles from './ReviewsRating2.module.css';

const ReviewsRating2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsRating2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsRating2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsRating2Schema.safeParse({ value: val });
