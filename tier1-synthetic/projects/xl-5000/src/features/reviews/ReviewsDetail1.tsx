...
import ReviewsInfiniteScroll1 from './ReviewsInfiniteScroll1';
import ReviewsConfirm1 from './ReviewsConfirm1';
import styles from './ReviewsDetail1.module.css';

interface ReviewsDetail1Item {
  createdAt: Date;
  assignee: string;
  description: string;
  tags: string[];
...
interface ReviewsDetail1Props {
  items?: ReviewsDetail1Item[];
...
export default function ReviewsDetail1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'assignee', 'description']);
