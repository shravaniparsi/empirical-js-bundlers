...
import { z } from 'zod';
import ReviewsToggle1 from './ReviewsToggle1';
import ReviewsCollapse1 from './ReviewsCollapse1';
import styles from './ReviewsColorPicker2.module.css';

const ReviewsColorPicker2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsColorPicker2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsColorPicker2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsColorPicker2Schema.safeParse({ value: val });
