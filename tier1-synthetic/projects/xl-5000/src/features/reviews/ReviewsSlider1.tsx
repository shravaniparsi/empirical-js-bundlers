...
import styles from './ReviewsSlider1.module.css';

const ReviewsSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsSlider1Schema.safeParse({ value: val });
