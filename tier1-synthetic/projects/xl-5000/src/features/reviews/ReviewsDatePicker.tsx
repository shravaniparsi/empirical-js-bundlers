...
import styles from './ReviewsDatePicker.module.css';

const ReviewsDatePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsDatePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsDatePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsDatePickerSchema.safeParse({ value: val });
