...
import { z } from 'zod';
import ReviewsPopover1 from './ReviewsPopover1';
import ReviewsPrefetch from './ReviewsPrefetch';
import LocalizationHOC from '../localization/LocalizationHOC';
import styles from './ReviewsRadio.module.css';

const ReviewsRadioSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsRadioProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsRadio({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsRadioSchema.safeParse({ value: val });
