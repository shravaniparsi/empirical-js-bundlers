...
import styles from './FormsFileUpload.module.css';

const FormsFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FormsFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FormsFileUpload({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FormsFileUploadSchema.safeParse({ value: val });
