...
import { z } from 'zod';
import FormsInput1 from './FormsInput1';
import styles from './FormsAutocomplete2.module.css';

const FormsAutocomplete2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FormsAutocomplete2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FormsAutocomplete2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FormsAutocomplete2Schema.safeParse({ value: val });
