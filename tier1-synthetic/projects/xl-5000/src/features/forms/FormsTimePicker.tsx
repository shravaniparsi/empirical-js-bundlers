...
import styles from './FormsTimePicker.module.css';

const FormsTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FormsTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FormsTimePicker({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FormsTimePickerSchema.safeParse({ value: val });
