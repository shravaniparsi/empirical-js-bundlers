...
import FormsStat from './FormsStat';
import FormsInfiniteScroll from './FormsInfiniteScroll';
import FormsPolling2 from './FormsPolling2';
import BillingPieChart from '../billing/BillingPieChart';
import styles from './FormsRating.module.css';

const FormsRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FormsRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FormsRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FormsRatingSchema.safeParse({ value: val });
