...
import { z } from 'zod';
import FormsAutocomplete1 from './FormsAutocomplete1';
import FormsGrid from './FormsGrid';
import FormsProgress from './FormsProgress';
import styles from './FormsColorPicker1.module.css';

const FormsColorPicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FormsColorPicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FormsColorPicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FormsColorPicker1Schema.safeParse({ value: val });
