...
import styles from './FormsInput1.module.css';

const FormsInput1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FormsInput1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FormsInput1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FormsInput1Schema.safeParse({ value: val });
