...
import styles from './FormsRadio.module.css';

const FormsRadioSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FormsRadioProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FormsRadio({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FormsRadioSchema.safeParse({ value: val });
