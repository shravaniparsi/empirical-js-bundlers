...
import FormsRadio1 from './FormsRadio1';
import FormsCard1 from './FormsCard1';
import styles from './FormsColorPicker.module.css';

const FormsColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FormsColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FormsColorPicker({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FormsColorPickerSchema.safeParse({ value: val });
