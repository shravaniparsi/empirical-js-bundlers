...
import { z } from 'zod';
import FormsTimePicker from './FormsTimePicker';
import ActivityLabel from '../activity/ActivityLabel';
import styles from './FormsTimePicker1.module.css';

const FormsTimePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FormsTimePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FormsTimePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FormsTimePicker1Schema.safeParse({ value: val });
