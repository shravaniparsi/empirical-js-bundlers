...
import { z } from 'zod';
import FormsAreaChart1 from './FormsAreaChart1';
import FormsAutocomplete2 from './FormsAutocomplete2';
import styles from './FormsColorPicker2.module.css';

const FormsColorPicker2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FormsColorPicker2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FormsColorPicker2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FormsColorPicker2Schema.safeParse({ value: val });
