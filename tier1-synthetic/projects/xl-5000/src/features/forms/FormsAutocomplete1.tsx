...
import FormsAreaChart from './FormsAreaChart';
import FormsPagination from './FormsPagination';
import styles from './FormsAutocomplete1.module.css';

const FormsAutocomplete1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FormsAutocomplete1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FormsAutocomplete1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FormsAutocomplete1Schema.safeParse({ value: val });
