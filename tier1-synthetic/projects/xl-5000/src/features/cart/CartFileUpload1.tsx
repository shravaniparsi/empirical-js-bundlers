...
import CartPieChart1 from './CartPieChart1';
import styles from './CartFileUpload1.module.css';

const CartFileUpload1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CartFileUpload1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CartFileUpload1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CartFileUpload1Schema.safeParse({ value: val });
