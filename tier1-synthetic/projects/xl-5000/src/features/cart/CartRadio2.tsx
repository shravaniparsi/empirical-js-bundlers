...
import styles from './CartRadio2.module.css';

const CartRadio2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CartRadio2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CartRadio2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CartRadio2Schema.safeParse({ value: val });
