...
import { z } from 'zod';
import CartRank from './CartRank';
import CartSearch from './CartSearch';
import styles from './CartRangeSlider.module.css';

const CartRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CartRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CartRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CartRangeSliderSchema.safeParse({ value: val });
