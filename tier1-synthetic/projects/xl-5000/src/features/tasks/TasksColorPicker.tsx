...
import styles from './TasksColorPicker.module.css';

const TasksColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TasksColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TasksColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TasksColorPickerSchema.safeParse({ value: val });
