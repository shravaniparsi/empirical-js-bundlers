...
import styles from './TasksFileUpload.module.css';

const TasksFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TasksFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TasksFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TasksFileUploadSchema.safeParse({ value: val });
