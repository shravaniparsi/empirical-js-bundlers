...
import styles from './TasksLabel1.module.css';

interface TasksLabel1Item {
  priority: number;
  title: string;
  description: string;
  tags: string[];
...
interface TasksLabel1Props {
  items?: TasksLabel1Item[];
...
export default function TasksLabel1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['priority', 'title', 'description']);
