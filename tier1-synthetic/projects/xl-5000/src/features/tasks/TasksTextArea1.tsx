...
import TasksStepper from './TasksStepper';
import styles from './TasksTextArea1.module.css';

const TasksTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TasksTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TasksTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TasksTextArea1Schema.safeParse({ value: val });
