...
import { z } from 'zod';
import OrdersResponsive from './OrdersResponsive';
import WorkflowsCard from '../workflows/WorkflowsCard';
import styles from './OrdersTextArea.module.css';

const OrdersTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface OrdersTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function OrdersTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = OrdersTextAreaSchema.safeParse({ value: val });
