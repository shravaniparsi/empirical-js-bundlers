...
import styles from './OrdersTextArea1.module.css';

const OrdersTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface OrdersTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function OrdersTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = OrdersTextArea1Schema.safeParse({ value: val });
