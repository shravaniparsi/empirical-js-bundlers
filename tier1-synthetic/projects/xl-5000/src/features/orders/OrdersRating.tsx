...
import { z } from 'zod';
import OrdersSuspense1 from './OrdersSuspense1';
import styles from './OrdersRating.module.css';

const OrdersRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface OrdersRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function OrdersRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = OrdersRatingSchema.safeParse({ value: val });
