...
import { z } from 'zod';
import OrdersHeatmap1 from './OrdersHeatmap1';
import OrdersTextArea from './OrdersTextArea';
import OrdersBreadcrumb from './OrdersBreadcrumb';
import AnalyticsList from '../analytics/AnalyticsList';
import styles from './OrdersSlider1.module.css';

const OrdersSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface OrdersSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function OrdersSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = OrdersSlider1Schema.safeParse({ value: val });
