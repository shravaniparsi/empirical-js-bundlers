...
import { z } from 'zod';
import OrdersPanel1 from './OrdersPanel1';
import OrdersBottomNav1 from './OrdersBottomNav1';
import FeedbackMgmtSummary from '../feedback-mgmt/FeedbackMgmtSummary';
import styles from './OrdersCheckbox.module.css';

const OrdersCheckboxSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface OrdersCheckboxProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function OrdersCheckbox({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = OrdersCheckboxSchema.safeParse({ value: val });
