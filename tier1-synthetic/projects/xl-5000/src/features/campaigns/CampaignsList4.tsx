...
import styles from './CampaignsList4.module.css';

interface CampaignsList4Item {
  tags: string[];
  description: string;
  assignee: string;
  id: string;
...
interface CampaignsList4Props {
  items?: CampaignsList4Item[];
...
export default function CampaignsList4({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'description', 'assignee']);
