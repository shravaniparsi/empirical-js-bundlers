...
import styles from './CampaignsTextArea.module.css';

const CampaignsTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CampaignsTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CampaignsTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CampaignsTextAreaSchema.safeParse({ value: val });
