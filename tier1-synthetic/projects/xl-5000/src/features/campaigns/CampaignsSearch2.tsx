...
import CampaignsProgress1 from './CampaignsProgress1';
import NotificationsContext from '../notifications/NotificationsContext';
import styles from './CampaignsSearch2.module.css';

const CampaignsSearch2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CampaignsSearch2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CampaignsSearch2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CampaignsSearch2Schema.safeParse({ value: val });
