...
import CampaignsList3 from './CampaignsList3';
import CampaignsRating from './CampaignsRating';
import CampaignsScroll1 from './CampaignsScroll1';
import AuthList from '../auth/AuthList';
import DocumentsCard from '../documents/DocumentsCard';
import styles from './CampaignsProgress.module.css';

interface CampaignsProgressItem {
  createdAt: Date;
  description: string;
  tags: string[];
  assignee: string;
...
interface CampaignsProgressProps {
  items?: CampaignsProgressItem[];
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'description', 'tags']);
