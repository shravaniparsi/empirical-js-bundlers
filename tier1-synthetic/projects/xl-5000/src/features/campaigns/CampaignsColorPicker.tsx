...
import CampaignsBridge from './CampaignsBridge';
import styles from './CampaignsColorPicker.module.css';

const CampaignsColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CampaignsColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CampaignsColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CampaignsColorPickerSchema.safeParse({ value: val });
