...
import styles from './CampaignsDatePicker.module.css';

const CampaignsDatePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CampaignsDatePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CampaignsDatePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CampaignsDatePickerSchema.safeParse({ value: val });
