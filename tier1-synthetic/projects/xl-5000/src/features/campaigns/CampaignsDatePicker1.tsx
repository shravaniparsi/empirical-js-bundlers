...
import styles from './CampaignsDatePicker1.module.css';

const CampaignsDatePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CampaignsDatePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CampaignsDatePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CampaignsDatePicker1Schema.safeParse({ value: val });
