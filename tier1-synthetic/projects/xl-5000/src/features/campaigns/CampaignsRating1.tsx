...
import CampaignsSnackbar from './CampaignsSnackbar';
import PaymentsSidebar from '../payments/PaymentsSidebar';
import styles from './CampaignsRating1.module.css';

const CampaignsRating1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CampaignsRating1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CampaignsRating1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CampaignsRating1Schema.safeParse({ value: val });
