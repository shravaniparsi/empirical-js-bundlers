...
import { z } from 'zod';
import CampaignsChip from './CampaignsChip';
import OrdersSticky1 from '../orders/OrdersSticky1';
import styles from './CampaignsCheckbox.module.css';

const CampaignsCheckboxSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CampaignsCheckboxProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CampaignsCheckbox({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CampaignsCheckboxSchema.safeParse({ value: val });
