...
import CampaignsProgress from './CampaignsProgress';
import styles from './CampaignsBadge2.module.css';

interface CampaignsBadge2Item {
  createdAt: Date;
  priority: number;
  description: string;
  id: string;
...
interface CampaignsBadge2Props {
  items?: CampaignsBadge2Item[];
...
export default function CampaignsBadge2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: CampaignsBadge2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'priority', 'description']);
