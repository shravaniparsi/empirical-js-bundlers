...
import { z } from 'zod';
import CampaignsDetail1 from './CampaignsDetail1';
import CampaignsSidebar from './CampaignsSidebar';
import styles from './CampaignsSearch1.module.css';

const CampaignsSearch1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CampaignsSearch1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CampaignsSearch1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CampaignsSearch1Schema.safeParse({ value: val });
