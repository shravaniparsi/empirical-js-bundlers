...
import styles from './CampaignsProgress1.module.css';

interface CampaignsProgress1Item {
  description: string;
  updatedAt: Date;
  tags: string[];
  createdAt: Date;
...
interface CampaignsProgress1Props {
  items?: CampaignsProgress1Item[];
...
export default function CampaignsProgress1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: CampaignsProgress1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'updatedAt', 'tags']);
