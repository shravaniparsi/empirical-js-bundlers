...
import styles from './ReportsMeter1.module.css';

interface ReportsMeter1Item {
  status: string;
  description: string;
  assignee: string;
  priority: number;
...
interface ReportsMeter1Props {
  items?: ReportsMeter1Item[];
...
export default function ReportsMeter1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'description', 'assignee']);
