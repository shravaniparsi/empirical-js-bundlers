...
import styles from './ReportsColorPicker1.module.css';

const ReportsColorPicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReportsColorPicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReportsColorPicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReportsColorPicker1Schema.safeParse({ value: val });
