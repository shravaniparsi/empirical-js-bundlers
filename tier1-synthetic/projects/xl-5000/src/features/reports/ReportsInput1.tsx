...
import styles from './ReportsInput1.module.css';

const ReportsInput1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReportsInput1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReportsInput1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReportsInput1Schema.safeParse({ value: val });
