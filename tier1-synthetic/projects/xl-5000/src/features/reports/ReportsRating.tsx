...
import { z } from 'zod';
import ReportsBanner2 from './ReportsBanner2';
import ReportsToast from './ReportsToast';
import ReportsPopover2 from './ReportsPopover2';
import AuthList from '../auth/AuthList';
import styles from './ReportsRating.module.css';

const ReportsRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReportsRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReportsRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReportsRatingSchema.safeParse({ value: val });
