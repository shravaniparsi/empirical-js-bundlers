...
import styles from './ReportsAutocomplete.module.css';

const ReportsAutocompleteSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReportsAutocompleteProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReportsAutocomplete({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReportsAutocompleteSchema.safeParse({ value: val });
