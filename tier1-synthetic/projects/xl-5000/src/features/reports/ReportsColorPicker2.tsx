...
import { z } from 'zod';
import ReportsBridge from './ReportsBridge';
import styles from './ReportsColorPicker2.module.css';

const ReportsColorPicker2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReportsColorPicker2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReportsColorPicker2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReportsColorPicker2Schema.safeParse({ value: val });
