...
import styles from './ReportsTextArea1.module.css';

const ReportsTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReportsTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReportsTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReportsTextArea1Schema.safeParse({ value: val });
