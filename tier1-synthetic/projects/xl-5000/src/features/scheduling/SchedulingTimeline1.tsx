...
import styles from './SchedulingTimeline1.module.css';

interface SchedulingTimeline1Item {
  assignee: string;
  createdAt: Date;
  tags: string[];
  updatedAt: Date;
...
interface SchedulingTimeline1Props {
  items?: SchedulingTimeline1Item[];
...
export default function SchedulingTimeline1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: SchedulingTimeline1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'createdAt', 'tags']);
