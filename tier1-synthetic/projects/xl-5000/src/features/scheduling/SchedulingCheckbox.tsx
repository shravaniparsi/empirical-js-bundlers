...
import CheckoutAlert1 from '../checkout/CheckoutAlert1';
import styles from './SchedulingCheckbox.module.css';

const SchedulingCheckboxSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SchedulingCheckboxProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SchedulingCheckbox({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SchedulingCheckboxSchema.safeParse({ value: val });
