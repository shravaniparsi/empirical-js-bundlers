...
import styles from './SchedulingAutocomplete1.module.css';

const SchedulingAutocomplete1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SchedulingAutocomplete1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SchedulingAutocomplete1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SchedulingAutocomplete1Schema.safeParse({ value: val });
