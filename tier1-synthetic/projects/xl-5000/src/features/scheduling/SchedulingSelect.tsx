...
import { z } from 'zod';
import SchedulingGauge from './SchedulingGauge';
import SurveysDetail from '../surveys/SurveysDetail';
import styles from './SchedulingSelect.module.css';

const SchedulingSelectSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SchedulingSelectProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SchedulingSelect({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SchedulingSelectSchema.safeParse({ value: val });
