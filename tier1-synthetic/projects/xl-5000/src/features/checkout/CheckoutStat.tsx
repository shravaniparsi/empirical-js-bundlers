...
import CheckoutBottomNav from './CheckoutBottomNav';
import CheckoutHeatmap1 from './CheckoutHeatmap1';
import styles from './CheckoutStat.module.css';

interface CheckoutStatItem {
  status: string;
  updatedAt: Date;
  createdAt: Date;
  tags: string[];
...
export default function CheckoutStat({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'updatedAt', 'createdAt']);
