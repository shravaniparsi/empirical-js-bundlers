...
import { z } from 'zod';
import CheckoutScroll from './CheckoutScroll';
import CheckoutProgress1 from './CheckoutProgress1';
import styles from './CheckoutRating1.module.css';

const CheckoutRating1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CheckoutRating1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CheckoutRating1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CheckoutRating1Schema.safeParse({ value: val });
