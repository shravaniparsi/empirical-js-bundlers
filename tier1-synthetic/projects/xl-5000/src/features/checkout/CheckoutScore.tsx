...
import CheckoutPagination1 from './CheckoutPagination1';
import CheckoutPanel from './CheckoutPanel';
import ProjectsTimePicker from '../projects/ProjectsTimePicker';
import ShippingSpinner from '../shipping/ShippingSpinner';
import styles from './CheckoutScore.module.css';

interface CheckoutScoreItem {
  createdAt: Date;
  description: string;
  updatedAt: Date;
  title: string;
...
interface CheckoutScoreProps {
  items?: CheckoutScoreItem[];
...
export default function CheckoutScore({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'description', 'updatedAt']);
