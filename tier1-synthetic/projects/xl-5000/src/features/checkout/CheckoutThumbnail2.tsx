...
import styles from './CheckoutThumbnail2.module.css';

interface CheckoutThumbnail2Item {
  title: string;
  status: string;
  tags: string[];
  description: string;
...
interface CheckoutThumbnail2Props {
  items?: CheckoutThumbnail2Item[];
...
export default function CheckoutThumbnail2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: CheckoutThumbnail2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'status', 'tags']);
