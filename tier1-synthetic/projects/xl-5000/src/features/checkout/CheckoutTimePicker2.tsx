...
import { z } from 'zod';
import CheckoutColorPicker1 from './CheckoutColorPicker1';
import ReviewsDetail1 from '../reviews/ReviewsDetail1';
import styles from './CheckoutTimePicker2.module.css';

const CheckoutTimePicker2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CheckoutTimePicker2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CheckoutTimePicker2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CheckoutTimePicker2Schema.safeParse({ value: val });
