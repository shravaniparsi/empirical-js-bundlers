...
import NotificationsHeatmap from './NotificationsHeatmap';
import NotificationsChip from './NotificationsChip';
import NotificationsLabel2 from './NotificationsLabel2';
import styles from './NotificationsRank.module.css';

interface NotificationsRankItem {
  description: string;
  priority: number;
  title: string;
  tags: string[];
...
interface NotificationsRankProps {
  items?: NotificationsRankItem[];
...
export default function NotificationsRank({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: NotificationsRankProps) {
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [sortField, setSortField] = useState<'description' | 'priority'>('description');
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'priority', 'title']);
