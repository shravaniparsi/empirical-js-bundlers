...
import styles from './NotificationsRadio1.module.css';

const NotificationsRadio1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface NotificationsRadio1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function NotificationsRadio1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = NotificationsRadio1Schema.safeParse({ value: val });
