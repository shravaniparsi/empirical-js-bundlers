...
import NotificationsAdapter1 from './NotificationsAdapter1';
import NotificationsInfiniteScroll from './NotificationsInfiniteScroll';
import NotificationsDivider from './NotificationsDivider';
import AuthList from '../auth/AuthList';
import styles from './NotificationsScore1.module.css';

interface NotificationsScore1Item {
  tags: string[];
  title: string;
  createdAt: Date;
  assignee: string;
...
interface NotificationsScore1Props {
  items?: NotificationsScore1Item[];
...
export default function NotificationsScore1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: NotificationsScore1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'title', 'createdAt']);
