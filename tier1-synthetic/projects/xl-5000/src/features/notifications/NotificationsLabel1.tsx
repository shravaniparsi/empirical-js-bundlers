...
import NotificationsList2 from './NotificationsList2';
import NotificationsDivider1 from './NotificationsDivider1';
import NotificationsTextArea from './NotificationsTextArea';
import ReportsMeter1 from '../reports/ReportsMeter1';
import styles from './NotificationsLabel1.module.css';

interface NotificationsLabel1Item {
  createdAt: Date;
  title: string;
  description: string;
  priority: number;
...
interface NotificationsLabel1Props {
  items?: NotificationsLabel1Item[];
...
export default function NotificationsLabel1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: NotificationsLabel1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'title', 'description']);
