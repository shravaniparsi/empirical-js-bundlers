...
import NotificationsInfiniteScroll1 from './NotificationsInfiniteScroll1';
import styles from './NotificationsRank1.module.css';

interface NotificationsRank1Item {
  title: string;
  status: string;
  updatedAt: Date;
  createdAt: Date;
...
interface NotificationsRank1Props {
  items?: NotificationsRank1Item[];
...
export default function NotificationsRank1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: NotificationsRank1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'status', 'updatedAt']);
