...
import NotificationsPaginated from './NotificationsPaginated';
import NotificationsGrid5 from './NotificationsGrid5';
import WorkflowsFactory from '../workflows/WorkflowsFactory';
import styles from './NotificationsInfiniteScroll1.module.css';
...
interface NotificationsInfiniteScroll1Props<T = unknown> {
  url?: string;
  fetchFn?: () => Promise<T>;
...
export default function NotificationsInfiniteScroll1<T = unknown>({
  url,
  fetchFn,
  children: renderProp,
  retryCount = 3,
  ...
  errorFallback,
  className,
}: NotificationsInfiniteScroll1Props<T>) {
