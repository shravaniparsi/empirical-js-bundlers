...
import NotificationsRadio from './NotificationsRadio';
import styles from './NotificationsChip1.module.css';

interface NotificationsChip1Item {
  description: string;
  status: string;
  title: string;
  tags: string[];
...
interface NotificationsChip1Props {
  items?: NotificationsChip1Item[];
...
export default function NotificationsChip1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: NotificationsChip1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'status', 'title']);
