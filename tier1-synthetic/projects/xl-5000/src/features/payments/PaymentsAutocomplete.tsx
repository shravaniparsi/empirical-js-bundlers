...
import PaymentsErrorBoundary1 from './PaymentsErrorBoundary1';
...
import styles from './PaymentsAutocomplete.module.css';

const PaymentsAutocompleteSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface PaymentsAutocompleteProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function PaymentsAutocomplete({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = PaymentsAutocompleteSchema.safeParse({ value: val });
