...
import { z } from 'zod';
import PaymentsThumbnail from './PaymentsThumbnail';
import styles from './PaymentsRadio1.module.css';

const PaymentsRadio1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface PaymentsRadio1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function PaymentsRadio1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = PaymentsRadio1Schema.safeParse({ value: val });
