...
import styles from './PaymentsPreview2.module.css';

interface PaymentsPreview2Item {
  status: string;
  assignee: string;
  priority: number;
  id: string;
...
interface PaymentsPreview2Props {
  items?: PaymentsPreview2Item[];
...
export default function PaymentsPreview2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: PaymentsPreview2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'assignee', 'priority']);
