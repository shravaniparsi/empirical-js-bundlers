...
import styles from './PaymentsSlider.module.css';

const PaymentsSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface PaymentsSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function PaymentsSlider({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = PaymentsSliderSchema.safeParse({ value: val });
