...
import styles from './PaymentsCheckbox.module.css';

const PaymentsCheckboxSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface PaymentsCheckboxProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function PaymentsCheckbox({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = PaymentsCheckboxSchema.safeParse({ value: val });
