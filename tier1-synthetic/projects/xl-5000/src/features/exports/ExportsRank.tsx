...
import ExportsThumbnail from './ExportsThumbnail';
import ExportsPieChart from './ExportsPieChart';
import ShippingTag1 from '../shipping/ShippingTag1';
import styles from './ExportsRank.module.css';

interface ExportsRankItem {
  status: string;
  tags: string[];
  title: string;
  priority: number;
...
interface ExportsRankProps {
  items?: ExportsRankItem[];
...
export default function ExportsRank({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'tags', 'title']);
