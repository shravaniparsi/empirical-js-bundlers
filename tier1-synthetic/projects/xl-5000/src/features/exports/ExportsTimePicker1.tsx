...
import styles from './ExportsTimePicker1.module.css';

const ExportsTimePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ExportsTimePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ExportsTimePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ExportsTimePicker1Schema.safeParse({ value: val });
