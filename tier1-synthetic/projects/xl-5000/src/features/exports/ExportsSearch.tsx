...
import IntegrationsPlaceholder1 from '../integrations/IntegrationsPlaceholder1';
import styles from './ExportsSearch.module.css';

const ExportsSearchSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ExportsSearchProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ExportsSearch({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ExportsSearchSchema.safeParse({ value: val });
