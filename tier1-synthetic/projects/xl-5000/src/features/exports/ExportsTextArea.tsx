...
import { z } from 'zod';
import ExportsStack from './ExportsStack';
import OrdersHeatmap1 from '../orders/OrdersHeatmap1';
import styles from './ExportsTextArea.module.css';

const ExportsTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ExportsTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ExportsTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ExportsTextAreaSchema.safeParse({ value: val });
