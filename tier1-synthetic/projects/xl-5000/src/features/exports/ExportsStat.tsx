...
import ExportsContainer3 from './ExportsContainer3';
import ExportsChip from './ExportsChip';
import ReviewsErrorBoundary from '../reviews/ReviewsErrorBoundary';
import styles from './ExportsStat.module.css';

interface ExportsStatItem {
  tags: string[];
  assignee: string;
  createdAt: Date;
  updatedAt: Date;
...
interface ExportsStatProps {
  items?: ExportsStatItem[];
...
export default function ExportsStat({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'assignee', 'createdAt']);
