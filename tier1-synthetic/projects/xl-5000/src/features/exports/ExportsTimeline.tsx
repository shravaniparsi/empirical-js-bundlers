...
import ExportsBridge from './ExportsBridge';
import CommentsResponsive1 from '../comments/CommentsResponsive1';
import styles from './ExportsTimeline.module.css';

interface ExportsTimelineItem {
  tags: string[];
  createdAt: Date;
  description: string;
  id: string;
...
interface ExportsTimelineProps {
  items?: ExportsTimelineItem[];
...
export default function ExportsTimeline({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: ExportsTimelineProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'createdAt', 'description']);
