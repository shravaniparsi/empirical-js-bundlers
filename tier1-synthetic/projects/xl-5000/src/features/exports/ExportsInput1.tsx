...
import { z } from 'zod';
import ExportsAreaChart from './ExportsAreaChart';
import ExportsLoader1 from './ExportsLoader1';
import CategoriesAdapter1 from '../categories/CategoriesAdapter1';
import styles from './ExportsInput1.module.css';

const ExportsInput1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ExportsInput1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ExportsInput1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ExportsInput1Schema.safeParse({ value: val });
