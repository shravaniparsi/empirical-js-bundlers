...
import styles from './ExportsRangeSlider2.module.css';

const ExportsRangeSlider2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ExportsRangeSlider2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ExportsRangeSlider2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ExportsRangeSlider2Schema.safeParse({ value: val });
