...
import styles from './ExportsFileUpload1.module.css';

const ExportsFileUpload1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ExportsFileUpload1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ExportsFileUpload1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ExportsFileUpload1Schema.safeParse({ value: val });
