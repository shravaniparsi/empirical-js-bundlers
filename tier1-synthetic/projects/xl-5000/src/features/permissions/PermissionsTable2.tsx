...
import TasksToast from '../tasks/TasksToast';
import styles from './PermissionsTable2.module.css';

interface PermissionsTable2Item {
  assignee: string;
  priority: number;
  title: string;
  status: string;
...
interface PermissionsTable2Props {
  items?: PermissionsTable2Item[];
...
export default function PermissionsTable2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: PermissionsTable2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'priority', 'title']);
