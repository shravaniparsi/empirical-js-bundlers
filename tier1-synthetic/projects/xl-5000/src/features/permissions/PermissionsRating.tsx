...
import styles from './PermissionsRating.module.css';

const PermissionsRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface PermissionsRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function PermissionsRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = PermissionsRatingSchema.safeParse({ value: val });
