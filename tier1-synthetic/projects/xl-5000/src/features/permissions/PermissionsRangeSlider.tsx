...
import styles from './PermissionsRangeSlider.module.css';

const PermissionsRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface PermissionsRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function PermissionsRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = PermissionsRangeSliderSchema.safeParse({ value: val });
