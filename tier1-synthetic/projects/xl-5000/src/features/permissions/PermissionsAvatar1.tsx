...
import PermissionsFeed from './PermissionsFeed';
import styles from './PermissionsAvatar1.module.css';

interface PermissionsAvatar1Item {
  createdAt: Date;
  tags: string[];
  updatedAt: Date;
  id: string;
...
interface PermissionsAvatar1Props {
  items?: PermissionsAvatar1Item[];
...
export default function PermissionsAvatar1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: PermissionsAvatar1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'tags', 'updatedAt']);
