...
import PermissionsGauge from './PermissionsGauge';
import PermissionsDonut1 from './PermissionsDonut1';
import styles from './PermissionsStat1.module.css';

interface PermissionsStat1Item {
  priority: number;
  tags: string[];
  assignee: string;
  createdAt: Date;
...
interface PermissionsStat1Props {
  items?: PermissionsStat1Item[];
...
export default function PermissionsStat1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: PermissionsStat1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['priority', 'tags', 'assignee']);
