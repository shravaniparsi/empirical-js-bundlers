...
import styles from './CategoriesTextArea.module.css';

const CategoriesTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CategoriesTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CategoriesTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CategoriesTextAreaSchema.safeParse({ value: val });
