...
import CategoriesCard1 from './CategoriesCard1';
import styles from './CategoriesDatePicker.module.css';

const CategoriesDatePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CategoriesDatePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CategoriesDatePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CategoriesDatePickerSchema.safeParse({ value: val });
