...
import CategoriesContainer1 from './CategoriesContainer1';
import styles from './CategoriesGrid1.module.css';

interface CategoriesGrid1Item {
  title: string;
  description: string;
  createdAt: Date;
  id: string;
...
interface CategoriesGrid1Props {
  items?: CategoriesGrid1Item[];
...
export default function CategoriesGrid1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: CategoriesGrid1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'description', 'createdAt']);
