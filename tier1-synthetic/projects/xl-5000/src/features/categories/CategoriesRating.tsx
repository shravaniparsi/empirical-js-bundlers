...
import { z } from 'zod';
import CategoriesScroll1 from './CategoriesScroll1';
import CategoriesInput from './CategoriesInput';
import styles from './CategoriesRating.module.css';

const CategoriesRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CategoriesRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CategoriesRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CategoriesRatingSchema.safeParse({ value: val });
