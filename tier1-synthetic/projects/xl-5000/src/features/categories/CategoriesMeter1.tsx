...
import CategoriesSparkline1 from './CategoriesSparkline1';
import AuthList from '../auth/AuthList';
import styles from './CategoriesMeter1.module.css';

interface CategoriesMeter1Item {
  description: string;
  tags: string[];
  createdAt: Date;
  id: string;
...
interface CategoriesMeter1Props {
  items?: CategoriesMeter1Item[];
...
export default function CategoriesMeter1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: CategoriesMeter1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'tags', 'createdAt']);
