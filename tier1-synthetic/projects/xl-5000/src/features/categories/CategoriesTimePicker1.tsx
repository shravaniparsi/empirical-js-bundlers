...
import CategoriesRating from './CategoriesRating';
import PermissionsSelect from '../permissions/PermissionsSelect';
import styles from './CategoriesTimePicker1.module.css';

const CategoriesTimePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CategoriesTimePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CategoriesTimePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CategoriesTimePicker1Schema.safeParse({ value: val });
