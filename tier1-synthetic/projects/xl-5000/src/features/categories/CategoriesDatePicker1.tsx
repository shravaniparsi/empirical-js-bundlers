...
import styles from './CategoriesDatePicker1.module.css';

const CategoriesDatePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CategoriesDatePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CategoriesDatePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CategoriesDatePicker1Schema.safeParse({ value: val });
