...
import CategoriesPrefetch from './CategoriesPrefetch';
import CategoriesRadio from './CategoriesRadio';
import LocalizationRadio from '../localization/LocalizationRadio';
import styles from './CategoriesColorPicker.module.css';

const CategoriesColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CategoriesColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CategoriesColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CategoriesColorPickerSchema.safeParse({ value: val });
