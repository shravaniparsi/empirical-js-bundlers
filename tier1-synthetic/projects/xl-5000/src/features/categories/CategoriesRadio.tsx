...
import styles from './CategoriesRadio.module.css';

const CategoriesRadioSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CategoriesRadioProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CategoriesRadio({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CategoriesRadioSchema.safeParse({ value: val });
