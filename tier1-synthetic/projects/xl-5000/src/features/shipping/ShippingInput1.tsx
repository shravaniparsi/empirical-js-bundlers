...
import { z } from 'zod';
import ShippingTreeView from './ShippingTreeView';
import styles from './ShippingInput1.module.css';

const ShippingInput1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ShippingInput1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ShippingInput1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ShippingInput1Schema.safeParse({ value: val });
