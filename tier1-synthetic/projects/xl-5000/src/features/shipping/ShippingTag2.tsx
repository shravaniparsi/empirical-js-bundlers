...
import ShippingRetry from './ShippingRetry';
import ShippingTable1 from './ShippingTable1';
import TasksToast from '../tasks/TasksToast';
import styles from './ShippingTag2.module.css';

interface ShippingTag2Item {
  updatedAt: Date;
  tags: string[];
  createdAt: Date;
  id: string;
...
interface ShippingTag2Props {
  items?: ShippingTag2Item[];
...
export default function ShippingTag2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'tags', 'createdAt']);
