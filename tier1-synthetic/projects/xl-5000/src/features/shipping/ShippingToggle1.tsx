...
import styles from './ShippingToggle1.module.css';

const ShippingToggle1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ShippingToggle1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ShippingToggle1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ShippingToggle1Schema.safeParse({ value: val });
