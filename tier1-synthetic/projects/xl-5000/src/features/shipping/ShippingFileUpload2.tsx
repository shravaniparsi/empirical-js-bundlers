...
import styles from './ShippingFileUpload2.module.css';

const ShippingFileUpload2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ShippingFileUpload2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ShippingFileUpload2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ShippingFileUpload2Schema.safeParse({ value: val });
