...
import styles from './ShippingColorPicker.module.css';

const ShippingColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ShippingColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ShippingColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ShippingColorPickerSchema.safeParse({ value: val });
