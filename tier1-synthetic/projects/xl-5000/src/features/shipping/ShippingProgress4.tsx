...
import ShippingBridge1 from './ShippingBridge1';
import ShippingSnackbar1 from './ShippingSnackbar1';
import styles from './ShippingProgress4.module.css';

interface ShippingProgress4Item {
  updatedAt: Date;
  tags: string[];
  assignee: string;
  description: string;
...
interface ShippingProgress4Props {
  items?: ShippingProgress4Item[];
...
export default function ShippingProgress4({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: ShippingProgress4Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'tags', 'assignee']);
