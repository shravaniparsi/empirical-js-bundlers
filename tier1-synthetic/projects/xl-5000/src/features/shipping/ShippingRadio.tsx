...
import styles from './ShippingRadio.module.css';

const ShippingRadioSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ShippingRadioProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ShippingRadio({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ShippingRadioSchema.safeParse({ value: val });
