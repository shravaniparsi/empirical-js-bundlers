...
import AuthGrid1 from './AuthGrid1';
import AuthSelect from './AuthSelect';
import AuthStepper1 from './AuthStepper1';
import AuthList from './AuthList';
import styles from './AuthScore1.module.css';

interface AuthScore1Item {
  title: string;
  tags: string[];
  createdAt: Date;
  priority: number;
...
interface AuthScore1Props {
  items?: AuthScore1Item[];
...
export default function AuthScore1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'tags', 'createdAt']);
