...
import { z } from 'zod';
import AuthDatePicker from './AuthDatePicker';
import AuthInput from './AuthInput';
import styles from './AuthFileUpload.module.css';

const AuthFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AuthFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AuthFileUpload({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AuthFileUploadSchema.safeParse({ value: val });
