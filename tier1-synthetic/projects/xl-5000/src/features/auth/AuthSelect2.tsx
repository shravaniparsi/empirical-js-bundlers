...
import styles from './AuthSelect2.module.css';

const AuthSelect2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AuthSelect2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AuthSelect2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AuthSelect2Schema.safeParse({ value: val });
