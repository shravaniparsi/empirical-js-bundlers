...
import styles from './SurveysFileUpload1.module.css';

const SurveysFileUpload1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SurveysFileUpload1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SurveysFileUpload1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SurveysFileUpload1Schema.safeParse({ value: val });
