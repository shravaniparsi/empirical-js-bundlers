...
import { z } from 'zod';
import SurveysFileUpload1 from './SurveysFileUpload1';
import SurveysTreeView from './SurveysTreeView';
import styles from './SurveysInput.module.css';

const SurveysInputSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SurveysInputProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SurveysInput({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SurveysInputSchema.safeParse({ value: val });
