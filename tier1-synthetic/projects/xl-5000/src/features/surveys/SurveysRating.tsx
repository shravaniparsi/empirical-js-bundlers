...
import { z } from 'zod';
import SurveysSidebar1 from './SurveysSidebar1';
import SurveysRegistry from './SurveysRegistry';
import styles from './SurveysRating.module.css';

const SurveysRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SurveysRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SurveysRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SurveysRatingSchema.safeParse({ value: val });
