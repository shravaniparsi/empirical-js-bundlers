...
import { z } from 'zod';
import SurveysAdapter from './SurveysAdapter';
import SurveysTimeline from './SurveysTimeline';
import SurveysSplit from './SurveysSplit';
import ReportsMeter1 from '../reports/ReportsMeter1';
import styles from './SurveysSelect.module.css';

const SurveysSelectSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SurveysSelectProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SurveysSelect({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SurveysSelectSchema.safeParse({ value: val });
