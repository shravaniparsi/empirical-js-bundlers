...
import { z } from 'zod';
import SurveysTimeline1 from './SurveysTimeline1';
import styles from './SurveysToggle.module.css';

const SurveysToggleSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SurveysToggleProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SurveysToggle({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SurveysToggleSchema.safeParse({ value: val });
