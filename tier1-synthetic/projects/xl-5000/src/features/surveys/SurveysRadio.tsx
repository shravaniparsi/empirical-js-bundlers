...
import styles from './SurveysRadio.module.css';

const SurveysRadioSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SurveysRadioProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SurveysRadio({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SurveysRadioSchema.safeParse({ value: val });
