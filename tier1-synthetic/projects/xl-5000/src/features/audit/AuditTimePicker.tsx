...
import styles from './AuditTimePicker.module.css';

const AuditTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AuditTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AuditTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AuditTimePickerSchema.safeParse({ value: val });
