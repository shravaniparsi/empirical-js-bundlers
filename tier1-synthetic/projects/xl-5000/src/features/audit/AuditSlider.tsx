...
import AuditRetry4 from './AuditRetry4';
import styles from './AuditSlider.module.css';

const AuditSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AuditSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AuditSlider({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AuditSliderSchema.safeParse({ value: val });
