...
import styles from './AuditColorPicker.module.css';

const AuditColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AuditColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AuditColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AuditColorPickerSchema.safeParse({ value: val });
