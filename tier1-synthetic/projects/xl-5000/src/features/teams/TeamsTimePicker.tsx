...
import styles from './TeamsTimePicker.module.css';

const TeamsTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TeamsTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TeamsTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TeamsTimePickerSchema.safeParse({ value: val });
