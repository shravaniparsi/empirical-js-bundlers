...
import styles from './TeamsSlider1.module.css';

const TeamsSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TeamsSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TeamsSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TeamsSlider1Schema.safeParse({ value: val });
