...
import { z } from 'zod';
import TeamsLoader2 from './TeamsLoader2';
import TeamsRangeSlider1 from './TeamsRangeSlider1';
import styles from './TeamsTextArea.module.css';

const TeamsTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TeamsTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TeamsTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TeamsTextAreaSchema.safeParse({ value: val });
