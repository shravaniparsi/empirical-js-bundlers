...
import TeamsDivider2 from './TeamsDivider2';
...
import styles from './TeamsAvatar2.module.css';

interface TeamsAvatar2Item {
  title: string;
  createdAt: Date;
  description: string;
  priority: number;
...
interface TeamsAvatar2Props {
  items?: TeamsAvatar2Item[];
...
export default function TeamsAvatar2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'createdAt', 'description']);
