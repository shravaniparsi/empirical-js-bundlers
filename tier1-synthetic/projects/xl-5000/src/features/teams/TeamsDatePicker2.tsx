...
import styles from './TeamsDatePicker2.module.css';

const TeamsDatePicker2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TeamsDatePicker2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TeamsDatePicker2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TeamsDatePicker2Schema.safeParse({ value: val });
