...
import TeamsProgress from './TeamsProgress';
import TeamsGauge from './TeamsGauge';
import styles from './TeamsThumbnail1.module.css';

interface TeamsThumbnail1Item {
  createdAt: Date;
  assignee: string;
  tags: string[];
  priority: number;
...
interface TeamsThumbnail1Props {
  items?: TeamsThumbnail1Item[];
...
export default function TeamsThumbnail1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: TeamsThumbnail1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'assignee', 'tags']);
