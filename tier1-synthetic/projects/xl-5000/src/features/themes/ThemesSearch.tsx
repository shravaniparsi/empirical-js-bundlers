...
import styles from './ThemesSearch.module.css';

const ThemesSearchSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ThemesSearchProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ThemesSearch({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ThemesSearchSchema.safeParse({ value: val });
