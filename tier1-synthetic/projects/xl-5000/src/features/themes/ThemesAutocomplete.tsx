...
import ThemesCard1 from './ThemesCard1';
import styles from './ThemesAutocomplete.module.css';

const ThemesAutocompleteSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ThemesAutocompleteProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ThemesAutocomplete({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ThemesAutocompleteSchema.safeParse({ value: val });
