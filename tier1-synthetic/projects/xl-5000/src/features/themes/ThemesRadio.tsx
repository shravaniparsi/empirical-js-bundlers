...
import { z } from 'zod';
import ThemesFileUpload from './ThemesFileUpload';
import ThemesComposer from './ThemesComposer';
import ThemesList1 from './ThemesList1';
import styles from './ThemesRadio.module.css';

const ThemesRadioSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ThemesRadioProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ThemesRadio({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ThemesRadioSchema.safeParse({ value: val });
