...
import ThemesScore from './ThemesScore';
import styles from './ThemesDatePicker.module.css';

const ThemesDatePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ThemesDatePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ThemesDatePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ThemesDatePickerSchema.safeParse({ value: val });
