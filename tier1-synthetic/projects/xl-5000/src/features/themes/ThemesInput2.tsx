...
import { z } from 'zod';
import ThemesTable1 from './ThemesTable1';
import ThemesProgress from './ThemesProgress';
import styles from './ThemesInput2.module.css';

const ThemesInput2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ThemesInput2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ThemesInput2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ThemesInput2Schema.safeParse({ value: val });
