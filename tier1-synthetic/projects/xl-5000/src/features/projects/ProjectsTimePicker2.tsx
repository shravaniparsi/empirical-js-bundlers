...
import { z } from 'zod';
import ProjectsConfirm from './ProjectsConfirm';
import ProjectsCached from './ProjectsCached';
import styles from './ProjectsTimePicker2.module.css';

const ProjectsTimePicker2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProjectsTimePicker2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProjectsTimePicker2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProjectsTimePicker2Schema.safeParse({ value: val });
