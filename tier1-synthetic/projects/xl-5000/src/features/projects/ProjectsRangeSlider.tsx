...
import { z } from 'zod';
import ProjectsThumbnail1 from './ProjectsThumbnail1';
import styles from './ProjectsRangeSlider.module.css';

const ProjectsRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProjectsRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProjectsRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProjectsRangeSliderSchema.safeParse({ value: val });
