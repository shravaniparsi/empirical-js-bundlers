...
import { z } from 'zod';
import ProjectsColorPicker from './ProjectsColorPicker';
import ProjectsLabel1 from './ProjectsLabel1';
import styles from './ProjectsToggle1.module.css';

const ProjectsToggle1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProjectsToggle1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProjectsToggle1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProjectsToggle1Schema.safeParse({ value: val });
