...
import { z } from 'zod';
import ProjectsFeed1 from './ProjectsFeed1';
import styles from './ProjectsFileUpload.module.css';

const ProjectsFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProjectsFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProjectsFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProjectsFileUploadSchema.safeParse({ value: val });
