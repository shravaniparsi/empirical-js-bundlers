...
import styles from './ProjectsFileUpload1.module.css';

const ProjectsFileUpload1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProjectsFileUpload1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProjectsFileUpload1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProjectsFileUpload1Schema.safeParse({ value: val });
