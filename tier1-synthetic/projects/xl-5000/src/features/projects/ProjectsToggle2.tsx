...
import styles from './ProjectsToggle2.module.css';

const ProjectsToggle2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProjectsToggle2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProjectsToggle2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProjectsToggle2Schema.safeParse({ value: val });
