...
import ProjectsPagination1 from './ProjectsPagination1';
import InventorySummary from '../inventory/InventorySummary';
import styles from './ProjectsRangeSlider1.module.css';

const ProjectsRangeSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProjectsRangeSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProjectsRangeSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProjectsRangeSlider1Schema.safeParse({ value: val });
