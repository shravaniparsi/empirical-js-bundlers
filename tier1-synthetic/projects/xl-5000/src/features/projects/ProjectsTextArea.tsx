...
import { z } from 'zod';
import ProjectsRetry from './ProjectsRetry';
import NotificationsLabel from '../notifications/NotificationsLabel';
import styles from './ProjectsTextArea.module.css';

const ProjectsTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProjectsTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProjectsTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProjectsTextAreaSchema.safeParse({ value: val });
