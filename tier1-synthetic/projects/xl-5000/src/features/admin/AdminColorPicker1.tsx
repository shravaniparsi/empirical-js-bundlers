...
import AdminAccordion1 from './AdminAccordion1';
import styles from './AdminColorPicker1.module.css';

const AdminColorPicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AdminColorPicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AdminColorPicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AdminColorPicker1Schema.safeParse({ value: val });
