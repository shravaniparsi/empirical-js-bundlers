...
import styles from './AdminCheckbox2.module.css';

const AdminCheckbox2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AdminCheckbox2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AdminCheckbox2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AdminCheckbox2Schema.safeParse({ value: val });
