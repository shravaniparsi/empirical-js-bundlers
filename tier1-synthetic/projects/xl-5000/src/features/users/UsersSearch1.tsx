...
import CheckoutSelect from '../checkout/CheckoutSelect';
import styles from './UsersSearch1.module.css';

const UsersSearch1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface UsersSearch1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function UsersSearch1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = UsersSearch1Schema.safeParse({ value: val });
