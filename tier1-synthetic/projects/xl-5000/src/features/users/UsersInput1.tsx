...
import { z } from 'zod';
import UsersFunnel from './UsersFunnel';
import UsersTabs from './UsersTabs';
import styles from './UsersInput1.module.css';

const UsersInput1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface UsersInput1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function UsersInput1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = UsersInput1Schema.safeParse({ value: val });
