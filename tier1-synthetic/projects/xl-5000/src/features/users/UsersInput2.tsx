...
import { z } from 'zod';
import UsersSnackbar2 from './UsersSnackbar2';
import RolesProgress1 from '../roles/RolesProgress1';
import styles from './UsersInput2.module.css';

const UsersInput2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface UsersInput2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function UsersInput2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = UsersInput2Schema.safeParse({ value: val });
