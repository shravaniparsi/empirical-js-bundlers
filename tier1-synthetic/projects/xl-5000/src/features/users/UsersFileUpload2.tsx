...
import { z } from 'zod';
import UsersPopover1 from './UsersPopover1';
import UsersPieChart1 from './UsersPieChart1';
import UsersList from './UsersList';
import LocalizationRadio from '../localization/LocalizationRadio';
import styles from './UsersFileUpload2.module.css';

const UsersFileUpload2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface UsersFileUpload2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function UsersFileUpload2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = UsersFileUpload2Schema.safeParse({ value: val });
