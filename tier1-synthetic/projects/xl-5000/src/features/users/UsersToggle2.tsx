...
import { z } from 'zod';
import UsersNotification1 from './UsersNotification1';
import UsersDetail1 from './UsersDetail1';
import styles from './UsersToggle2.module.css';

const UsersToggle2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface UsersToggle2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function UsersToggle2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = UsersToggle2Schema.safeParse({ value: val });
