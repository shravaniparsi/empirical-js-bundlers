...
import { z } from 'zod';
import LocalizationSelect1 from './LocalizationSelect1';
import styles from './LocalizationRangeSlider.module.css';

const LocalizationRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationRangeSliderSchema.safeParse({ value: val });
