...
import { z } from 'zod';
import LocalizationBarChart from './LocalizationBarChart';
import LocalizationSpacer from './LocalizationSpacer';
import LocalizationTimePicker from './LocalizationTimePicker';
import WebhooksSkeleton2 from '../webhooks/WebhooksSkeleton2';
import styles from './LocalizationRating1.module.css';

const LocalizationRating1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationRating1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationRating1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationRating1Schema.safeParse({ value: val });
  ...
  return (
    <div className={clsx(styles.fieldWrapper, disabled && styles.disabled, className)}>
      ...
      <LocalizationBarChart />
      <LocalizationSpacer />
      <LocalizationTimePicker />
      <WebhooksSkeleton2 />
      </div>
    </div>
  );
