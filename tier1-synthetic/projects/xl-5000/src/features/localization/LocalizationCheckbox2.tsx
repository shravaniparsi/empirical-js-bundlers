...
import { z } from 'zod';
import LocalizationSearch from './LocalizationSearch';
import LocalizationSidebar from './LocalizationSidebar';
import styles from './LocalizationCheckbox2.module.css';

const LocalizationCheckbox2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationCheckbox2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationCheckbox2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationCheckbox2Schema.safeParse({ value: val });
