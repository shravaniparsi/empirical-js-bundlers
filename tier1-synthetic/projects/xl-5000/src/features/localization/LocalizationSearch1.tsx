...
import styles from './LocalizationSearch1.module.css';

const LocalizationSearch1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationSearch1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationSearch1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationSearch1Schema.safeParse({ value: val });
