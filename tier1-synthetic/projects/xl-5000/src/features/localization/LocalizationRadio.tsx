...
import { z } from 'zod';
import LocalizationCheckbox1 from './LocalizationCheckbox1';
import styles from './LocalizationRadio.module.css';

const LocalizationRadioSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationRadioProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationRadio({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationRadioSchema.safeParse({ value: val });
