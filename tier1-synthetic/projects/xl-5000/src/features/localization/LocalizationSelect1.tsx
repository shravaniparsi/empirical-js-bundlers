...
import styles from './LocalizationSelect1.module.css';

const LocalizationSelect1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationSelect1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationSelect1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationSelect1Schema.safeParse({ value: val });
