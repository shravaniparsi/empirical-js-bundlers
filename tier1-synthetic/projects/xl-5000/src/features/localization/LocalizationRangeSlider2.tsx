...
import styles from './LocalizationRangeSlider2.module.css';

const LocalizationRangeSlider2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationRangeSlider2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationRangeSlider2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationRangeSlider2Schema.safeParse({ value: val });
