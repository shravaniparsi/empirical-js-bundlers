...
import { z } from 'zod';
import LocalizationTag1 from './LocalizationTag1';
import CommentsResponsive1 from '../comments/CommentsResponsive1';
import styles from './LocalizationRangeSlider1.module.css';

const LocalizationRangeSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationRangeSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationRangeSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationRangeSlider1Schema.safeParse({ value: val });
