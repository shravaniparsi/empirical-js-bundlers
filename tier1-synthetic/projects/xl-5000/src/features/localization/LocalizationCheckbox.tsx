...
import styles from './LocalizationCheckbox.module.css';

const LocalizationCheckboxSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationCheckboxProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationCheckbox({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationCheckboxSchema.safeParse({ value: val });
