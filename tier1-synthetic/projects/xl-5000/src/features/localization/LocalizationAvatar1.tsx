...
import styles from './LocalizationAvatar1.module.css';

interface LocalizationAvatar1Item {
  tags: string[];
  title: string;
  updatedAt: Date;
  createdAt: Date;
...
interface LocalizationAvatar1Props {
  items?: LocalizationAvatar1Item[];
...
export default function LocalizationAvatar1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: LocalizationAvatar1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'title', 'updatedAt']);
