...
import styles from './LocalizationCheckbox1.module.css';

const LocalizationCheckbox1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationCheckbox1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationCheckbox1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationCheckbox1Schema.safeParse({ value: val });
