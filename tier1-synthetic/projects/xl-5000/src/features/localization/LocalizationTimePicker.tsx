...
import styles from './LocalizationTimePicker.module.css';

const LocalizationTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationTimePickerSchema.safeParse({ value: val });
