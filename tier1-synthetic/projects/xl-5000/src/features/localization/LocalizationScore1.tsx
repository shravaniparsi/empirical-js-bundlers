...
import styles from './LocalizationScore1.module.css';

interface LocalizationScore1Item {
  status: string;
  tags: string[];
  updatedAt: Date;
  id: string;
...
interface LocalizationScore1Props {
  items?: LocalizationScore1Item[];
...
export default function LocalizationScore1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: LocalizationScore1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'tags', 'updatedAt']);
