...
import LocalizationFunnel from './LocalizationFunnel';
import ActivityTooltip from '../activity/ActivityTooltip';
import styles from './LocalizationFeed.module.css';

interface LocalizationFeedItem {
  status: string;
  createdAt: Date;
  description: string;
  tags: string[];
...
interface LocalizationFeedProps {
  items?: LocalizationFeedItem[];
...
export default function LocalizationFeed({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: LocalizationFeedProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'createdAt', 'description']);
