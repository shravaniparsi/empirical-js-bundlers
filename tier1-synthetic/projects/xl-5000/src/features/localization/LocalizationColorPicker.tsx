...
import { z } from 'zod';
import LocalizationToast from './LocalizationToast';
import TeamsCard1 from '../teams/TeamsCard1';
import styles from './LocalizationColorPicker.module.css';

const LocalizationColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationColorPickerSchema.safeParse({ value: val });
