...
import { z } from 'zod';
import LocalizationTag from './LocalizationTag';
import LocalizationRadio from './LocalizationRadio';
import styles from './LocalizationRadio2.module.css';

const LocalizationRadio2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface LocalizationRadio2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function LocalizationRadio2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = LocalizationRadio2Schema.safeParse({ value: val });
