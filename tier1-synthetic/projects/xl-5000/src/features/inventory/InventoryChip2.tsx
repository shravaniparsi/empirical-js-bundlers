...
import styles from './InventoryChip2.module.css';

interface InventoryChip2Item {
  tags: string[];
  priority: number;
  assignee: string;
  description: string;
...
interface InventoryChip2Props {
  items?: InventoryChip2Item[];
...
export default function InventoryChip2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'priority', 'assignee']);
