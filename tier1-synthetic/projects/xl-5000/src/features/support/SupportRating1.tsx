...
import SupportSpacer from './SupportSpacer';
import styles from './SupportRating1.module.css';

const SupportRating1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SupportRating1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SupportRating1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SupportRating1Schema.safeParse({ value: val });
