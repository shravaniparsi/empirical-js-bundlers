...
import SupportSnackbar from './SupportSnackbar';
import styles from './SupportRangeSlider1.module.css';

const SupportRangeSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SupportRangeSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SupportRangeSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SupportRangeSlider1Schema.safeParse({ value: val });
