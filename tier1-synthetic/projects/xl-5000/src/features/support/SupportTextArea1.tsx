...
import { z } from 'zod';
import SupportMenu1 from './SupportMenu1';
import SupportHeatmap from './SupportHeatmap';
import SupportSparkline from './SupportSparkline';
import styles from './SupportTextArea1.module.css';

const SupportTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SupportTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SupportTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SupportTextArea1Schema.safeParse({ value: val });
