...
import styles from './SupportTable2.module.css';

interface SupportTable2Item {
  updatedAt: Date;
  createdAt: Date;
  description: string;
  status: string;
...
interface SupportTable2Props {
  items?: SupportTable2Item[];
...
export default function SupportTable2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'createdAt', 'description']);
