...
import { z } from 'zod';
import SupportRating1 from './SupportRating1';
import styles from './SupportTimePicker.module.css';

const SupportTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SupportTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SupportTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SupportTimePickerSchema.safeParse({ value: val });
