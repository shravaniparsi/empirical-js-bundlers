...
import { z } from 'zod';
import SupportRank2 from './SupportRank2';
...
import styles from './SupportTimePicker2.module.css';

const SupportTimePicker2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SupportTimePicker2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SupportTimePicker2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SupportTimePicker2Schema.safeParse({ value: val });
