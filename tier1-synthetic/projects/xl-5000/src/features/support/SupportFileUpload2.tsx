...
import styles from './SupportFileUpload2.module.css';

const SupportFileUpload2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SupportFileUpload2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SupportFileUpload2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SupportFileUpload2Schema.safeParse({ value: val });
