...
import { z } from 'zod';
import SupportSpinner from './SupportSpinner';
import SupportRangeSlider from './SupportRangeSlider';
import styles from './SupportSlider2.module.css';

const SupportSlider2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SupportSlider2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SupportSlider2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SupportSlider2Schema.safeParse({ value: val });
