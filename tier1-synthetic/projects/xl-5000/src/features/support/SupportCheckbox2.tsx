...
import SupportGauge2 from './SupportGauge2';
import styles from './SupportCheckbox2.module.css';

const SupportCheckbox2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SupportCheckbox2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SupportCheckbox2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SupportCheckbox2Schema.safeParse({ value: val });
