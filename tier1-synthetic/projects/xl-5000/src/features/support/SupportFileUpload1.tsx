...
import { z } from 'zod';
import SupportContainer1 from './SupportContainer1';
import styles from './SupportFileUpload1.module.css';

const SupportFileUpload1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SupportFileUpload1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SupportFileUpload1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SupportFileUpload1Schema.safeParse({ value: val });
