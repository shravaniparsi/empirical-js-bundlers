...
import styles from './SettingsTimePicker1.module.css';

const SettingsTimePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SettingsTimePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SettingsTimePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SettingsTimePicker1Schema.safeParse({ value: val });
