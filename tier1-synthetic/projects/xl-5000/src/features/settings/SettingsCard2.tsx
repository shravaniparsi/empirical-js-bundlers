...
import SettingsTimePicker4 from './SettingsTimePicker4';
import SettingsToggle from './SettingsToggle';
import ApiKeysNotification1 from '../api-keys/ApiKeysNotification1';
import styles from './SettingsCard2.module.css';

interface SettingsCard2Item {
  description: string;
  createdAt: Date;
  assignee: string;
  updatedAt: Date;
...
interface SettingsCard2Props {
  items?: SettingsCard2Item[];
...
export default function SettingsCard2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'createdAt', 'assignee']);
