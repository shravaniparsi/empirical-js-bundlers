...
import styles from './SettingsTimePicker4.module.css';

const SettingsTimePicker4Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SettingsTimePicker4Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SettingsTimePicker4({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SettingsTimePicker4Schema.safeParse({ value: val });
