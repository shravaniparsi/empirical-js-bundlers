...
import styles from './DashboardSummary1.module.css';

interface DashboardSummary1Item {
  updatedAt: Date;
  priority: number;
  tags: string[];
  description: string;
...
interface DashboardSummary1Props {
  items?: DashboardSummary1Item[];
...
export default function DashboardSummary1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: DashboardSummary1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'priority', 'tags']);
