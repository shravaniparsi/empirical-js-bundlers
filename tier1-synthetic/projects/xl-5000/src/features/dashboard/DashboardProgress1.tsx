...
import DashboardRetry from './DashboardRetry';
import DashboardGrid3 from './DashboardGrid3';
import DashboardTable from './DashboardTable';
import AuditFeed from '../audit/AuditFeed';
import styles from './DashboardProgress1.module.css';

interface DashboardProgress1Item {
  tags: string[];
  status: string;
  updatedAt: Date;
  title: string;
...
interface DashboardProgress1Props {
  items?: DashboardProgress1Item[];
...
export default function DashboardProgress1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: DashboardProgress1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'status', 'updatedAt']);
