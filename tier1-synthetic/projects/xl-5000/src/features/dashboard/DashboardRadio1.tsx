...
import DashboardBottomNav1 from './DashboardBottomNav1';
import TeamsPreview from '../teams/TeamsPreview';
import styles from './DashboardRadio1.module.css';

const DashboardRadio1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DashboardRadio1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DashboardRadio1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DashboardRadio1Schema.safeParse({ value: val });
