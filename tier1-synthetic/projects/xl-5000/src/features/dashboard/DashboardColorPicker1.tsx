...
import styles from './DashboardColorPicker1.module.css';

const DashboardColorPicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DashboardColorPicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DashboardColorPicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DashboardColorPicker1Schema.safeParse({ value: val });
