...
import WorkflowsTimeline2 from './WorkflowsTimeline2';
import WorkflowsPreview1 from './WorkflowsPreview1';
import CampaignsSearch1 from '../campaigns/CampaignsSearch1';
import styles from './WorkflowsSlider1.module.css';

const WorkflowsSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsSlider1Schema.safeParse({ value: val });
