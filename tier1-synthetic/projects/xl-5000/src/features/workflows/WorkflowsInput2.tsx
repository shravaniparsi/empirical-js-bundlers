...
import { z } from 'zod';
import WorkflowsBridge from './WorkflowsBridge';
import WorkflowsRank1 from './WorkflowsRank1';
import WorkflowsTabs2 from './WorkflowsTabs2';
import MediaBadge from '../media/MediaBadge';
import styles from './WorkflowsInput2.module.css';

const WorkflowsInput2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsInput2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsInput2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsInput2Schema.safeParse({ value: val });
