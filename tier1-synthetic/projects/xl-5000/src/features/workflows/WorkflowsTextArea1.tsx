...
import { z } from 'zod';
import WorkflowsAvatar from './WorkflowsAvatar';
import BillingSpacer from '../billing/BillingSpacer';
import ShippingTag1 from '../shipping/ShippingTag1';
import styles from './WorkflowsTextArea1.module.css';

const WorkflowsTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsTextArea1Schema.safeParse({ value: val });
