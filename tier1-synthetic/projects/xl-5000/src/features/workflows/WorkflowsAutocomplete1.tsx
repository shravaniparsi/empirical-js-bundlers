...
import { z } from 'zod';
import WorkflowsColorPicker from './WorkflowsColorPicker';
import NotificationsScore from '../notifications/NotificationsScore';
import styles from './WorkflowsAutocomplete1.module.css';

const WorkflowsAutocomplete1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsAutocomplete1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsAutocomplete1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsAutocomplete1Schema.safeParse({ value: val });
