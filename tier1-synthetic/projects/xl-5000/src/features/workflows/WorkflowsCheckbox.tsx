...
import WorkflowsCached2 from './WorkflowsCached2';
import styles from './WorkflowsCheckbox.module.css';

const WorkflowsCheckboxSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsCheckboxProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsCheckbox({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsCheckboxSchema.safeParse({ value: val });
