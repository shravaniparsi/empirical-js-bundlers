...
import { z } from 'zod';
import WorkflowsAreaChart from './WorkflowsAreaChart';
import WorkflowsPopover1 from './WorkflowsPopover1';
import styles from './WorkflowsInput3.module.css';

const WorkflowsInput3Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsInput3Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsInput3({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsInput3Schema.safeParse({ value: val });
