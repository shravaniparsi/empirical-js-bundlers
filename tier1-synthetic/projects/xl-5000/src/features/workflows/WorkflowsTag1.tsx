...
import WorkflowsAutocomplete from './WorkflowsAutocomplete';
import WorkflowsSuspense from './WorkflowsSuspense';
import styles from './WorkflowsTag1.module.css';

interface WorkflowsTag1Item {
  assignee: string;
  priority: number;
  createdAt: Date;
  description: string;
...
interface WorkflowsTag1Props {
  items?: WorkflowsTag1Item[];
...
export default function WorkflowsTag1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'priority', 'createdAt']);
