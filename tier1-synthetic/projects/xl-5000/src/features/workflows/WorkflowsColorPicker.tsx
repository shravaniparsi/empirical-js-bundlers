...
import { z } from 'zod';
import WorkflowsResponsive from './WorkflowsResponsive';
import WorkflowsTimeline from './WorkflowsTimeline';
import styles from './WorkflowsColorPicker.module.css';

const WorkflowsColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsColorPickerSchema.safeParse({ value: val });
