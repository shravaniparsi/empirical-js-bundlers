...
import WorkflowsThumbnail from './WorkflowsThumbnail';
import styles from './WorkflowsDatePicker1.module.css';

const WorkflowsDatePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsDatePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsDatePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsDatePicker1Schema.safeParse({ value: val });
