...
import DocumentsBarChart2 from './DocumentsBarChart2';
import styles from './DocumentsLabel.module.css';

interface DocumentsLabelItem {
  status: string;
  updatedAt: Date;
  tags: string[];
  priority: number;
...
interface DocumentsLabelProps {
  items?: DocumentsLabelItem[];
...
export default function DocumentsLabel({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'updatedAt', 'tags']);
