...
import { z } from 'zod';
import DocumentsColorPicker from './DocumentsColorPicker';
import DocumentsProvider from './DocumentsProvider';
import PermissionsTreeView from '../permissions/PermissionsTreeView';
import styles from './DocumentsTimePicker.module.css';

const DocumentsTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DocumentsTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DocumentsTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DocumentsTimePickerSchema.safeParse({ value: val });
