...
import styles from './DocumentsDatePicker.module.css';

const DocumentsDatePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DocumentsDatePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DocumentsDatePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DocumentsDatePickerSchema.safeParse({ value: val });
