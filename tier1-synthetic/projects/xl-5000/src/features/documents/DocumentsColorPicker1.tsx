...
import { z } from 'zod';
import DocumentsComposer from './DocumentsComposer';
...
import styles from './DocumentsColorPicker1.module.css';

const DocumentsColorPicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DocumentsColorPicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DocumentsColorPicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DocumentsColorPicker1Schema.safeParse({ value: val });
