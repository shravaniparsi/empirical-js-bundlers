...
import { z } from 'zod';
import DocumentsScore1 from './DocumentsScore1';
import styles from './DocumentsColorPicker.module.css';

const DocumentsColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DocumentsColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DocumentsColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DocumentsColorPickerSchema.safeParse({ value: val });
