...
import styles from './DocumentsRadio.module.css';

const DocumentsRadioSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DocumentsRadioProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DocumentsRadio({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DocumentsRadioSchema.safeParse({ value: val });
