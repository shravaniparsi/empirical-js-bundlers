...
import { z } from 'zod';
import CatalogSuspense2 from './CatalogSuspense2';
import CatalogSidebar from './CatalogSidebar';
import ReviewsSpacer1 from '../reviews/ReviewsSpacer1';
import ThemesSticky1 from '../themes/ThemesSticky1';
import styles from './CatalogRangeSlider.module.css';

const CatalogRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CatalogRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CatalogRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CatalogRangeSliderSchema.safeParse({ value: val });
