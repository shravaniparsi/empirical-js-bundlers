...
import styles from './CatalogTimePicker.module.css';

const CatalogTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CatalogTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CatalogTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CatalogTimePickerSchema.safeParse({ value: val });
