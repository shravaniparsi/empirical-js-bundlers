...
import styles from './CatalogTextArea1.module.css';

const CatalogTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CatalogTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CatalogTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CatalogTextArea1Schema.safeParse({ value: val });
