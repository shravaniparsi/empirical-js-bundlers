...
import { z } from 'zod';
import CatalogModal1 from './CatalogModal1';
import styles from './CatalogTextArea.module.css';

const CatalogTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CatalogTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CatalogTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CatalogTextAreaSchema.safeParse({ value: val });
