...
import CatalogPreview1 from './CatalogPreview1';
import styles from './CatalogFileUpload.module.css';

const CatalogFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CatalogFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CatalogFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CatalogFileUploadSchema.safeParse({ value: val });
