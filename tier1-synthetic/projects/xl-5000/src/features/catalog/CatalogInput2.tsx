...
import styles from './CatalogInput2.module.css';

const CatalogInput2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CatalogInput2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CatalogInput2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CatalogInput2Schema.safeParse({ value: val });
