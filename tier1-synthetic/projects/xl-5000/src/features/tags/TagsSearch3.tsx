...
import { z } from 'zod';
import TagsBottomNav from './TagsBottomNav';
import TagsStat from './TagsStat';
import TagsScore from './TagsScore';
import styles from './TagsSearch3.module.css';

const TagsSearch3Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TagsSearch3Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TagsSearch3({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TagsSearch3Schema.safeParse({ value: val });
