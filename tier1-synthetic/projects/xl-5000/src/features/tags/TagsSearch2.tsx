...
import styles from './TagsSearch2.module.css';

const TagsSearch2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TagsSearch2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TagsSearch2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TagsSearch2Schema.safeParse({ value: val });
