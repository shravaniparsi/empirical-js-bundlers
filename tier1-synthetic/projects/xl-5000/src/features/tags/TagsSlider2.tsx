...
import { z } from 'zod';
import TagsPagination2 from './TagsPagination2';
import ThemesSparkline from '../themes/ThemesSparkline';
import styles from './TagsSlider2.module.css';

const TagsSlider2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TagsSlider2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TagsSlider2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TagsSlider2Schema.safeParse({ value: val });
