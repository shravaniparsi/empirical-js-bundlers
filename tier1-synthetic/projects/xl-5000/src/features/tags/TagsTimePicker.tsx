...
import CampaignsHook from '../campaigns/CampaignsHook';
import styles from './TagsTimePicker.module.css';

const TagsTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TagsTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TagsTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TagsTimePickerSchema.safeParse({ value: val });
