...
import { z } from 'zod';
import TagsAreaChart from './TagsAreaChart';
import styles from './TagsDatePicker1.module.css';

const TagsDatePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TagsDatePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TagsDatePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TagsDatePicker1Schema.safeParse({ value: val });
