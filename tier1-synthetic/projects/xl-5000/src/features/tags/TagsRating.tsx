...
import { z } from 'zod';
import TagsResponsive1 from './TagsResponsive1';
import styles from './TagsRating.module.css';

const TagsRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface TagsRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function TagsRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = TagsRatingSchema.safeParse({ value: val });
