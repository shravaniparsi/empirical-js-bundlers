...
import WebhooksAutocomplete from './WebhooksAutocomplete';
import WebhooksHeatmap1 from './WebhooksHeatmap1';
import WebhooksTabs from './WebhooksTabs';
import ApiKeysColorPicker1 from '../api-keys/ApiKeysColorPicker1';
import styles from './WebhooksList1.module.css';

interface WebhooksList1Item {
  description: string;
  tags: string[];
  status: string;
  id: string;
...
interface WebhooksList1Props {
  items?: WebhooksList1Item[];
...
export default function WebhooksList1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'tags', 'status']);
