...
import { z } from 'zod';
import WebhooksLoader2 from './WebhooksLoader2';
import WebhooksTag3 from './WebhooksTag3';
import styles from './WebhooksColorPicker.module.css';

const WebhooksColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WebhooksColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WebhooksColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WebhooksColorPickerSchema.safeParse({ value: val });
