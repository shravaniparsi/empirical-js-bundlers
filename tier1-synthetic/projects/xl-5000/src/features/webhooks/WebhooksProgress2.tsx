...
import WebhooksScatter from './WebhooksScatter';
import WebhooksSkeleton3 from './WebhooksSkeleton3';
import OrdersScore1 from '../orders/OrdersScore1';
import styles from './WebhooksProgress2.module.css';

interface WebhooksProgress2Item {
  tags: string[];
  title: string;
  description: string;
  updatedAt: Date;
...
interface WebhooksProgress2Props {
  items?: WebhooksProgress2Item[];
...
export default function WebhooksProgress2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: WebhooksProgress2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'title', 'description']);
