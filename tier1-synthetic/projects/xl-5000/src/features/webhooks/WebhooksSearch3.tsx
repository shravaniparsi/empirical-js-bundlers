...
import TemplatesDetail from '../templates/TemplatesDetail';
import styles from './WebhooksSearch3.module.css';

const WebhooksSearch3Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WebhooksSearch3Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WebhooksSearch3({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WebhooksSearch3Schema.safeParse({ value: val });
