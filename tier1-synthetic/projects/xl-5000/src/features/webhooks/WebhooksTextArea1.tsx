...
import styles from './WebhooksTextArea1.module.css';

const WebhooksTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WebhooksTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WebhooksTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WebhooksTextArea1Schema.safeParse({ value: val });
