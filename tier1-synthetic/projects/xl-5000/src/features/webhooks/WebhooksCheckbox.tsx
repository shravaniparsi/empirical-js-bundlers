...
import { z } from 'zod';
import WebhooksDetail1 from './WebhooksDetail1';
import WebhooksBanner from './WebhooksBanner';
import styles from './WebhooksCheckbox.module.css';

const WebhooksCheckboxSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WebhooksCheckboxProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WebhooksCheckbox({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WebhooksCheckboxSchema.safeParse({ value: val });
