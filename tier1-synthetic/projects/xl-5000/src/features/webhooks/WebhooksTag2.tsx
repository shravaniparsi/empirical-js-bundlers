...
import WebhooksTextArea from './WebhooksTextArea';
import WebhooksRetry from './WebhooksRetry';
import WebhooksMeter from './WebhooksMeter';
import styles from './WebhooksTag2.module.css';

interface WebhooksTag2Item {
  description: string;
  title: string;
  priority: number;
  status: string;
...
interface WebhooksTag2Props {
  items?: WebhooksTag2Item[];
...
export default function WebhooksTag2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'title', 'priority']);
