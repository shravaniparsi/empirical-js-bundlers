...
import { z } from 'zod';
import WebhooksThumbnail from './WebhooksThumbnail';
import styles from './WebhooksFileUpload.module.css';

const WebhooksFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WebhooksFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WebhooksFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WebhooksFileUploadSchema.safeParse({ value: val });
