...
import { z } from 'zod';
import WebhooksCached2 from './WebhooksCached2';
import WorkflowsNavBar from '../workflows/WorkflowsNavBar';
import styles from './WebhooksRangeSlider.module.css';

const WebhooksRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WebhooksRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WebhooksRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WebhooksRangeSliderSchema.safeParse({ value: val });
