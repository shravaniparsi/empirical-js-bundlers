...
import { z } from 'zod';
import WebhooksRangeSlider1 from './WebhooksRangeSlider1';
import WebhooksPieChart from './WebhooksPieChart';
import WebhooksProgress2 from './WebhooksProgress2';
import LocalizationLink1 from '../localization/LocalizationLink1';
import BillingRangeSlider1 from '../billing/BillingRangeSlider1';
import styles from './WebhooksSearch2.module.css';

const WebhooksSearch2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WebhooksSearch2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WebhooksSearch2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WebhooksSearch2Schema.safeParse({ value: val });
