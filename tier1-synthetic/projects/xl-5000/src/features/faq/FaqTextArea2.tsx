...
import { z } from 'zod';
import FaqRetry1 from './FaqRetry1';
import styles from './FaqTextArea2.module.css';

const FaqTextArea2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FaqTextArea2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FaqTextArea2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FaqTextArea2Schema.safeParse({ value: val });
