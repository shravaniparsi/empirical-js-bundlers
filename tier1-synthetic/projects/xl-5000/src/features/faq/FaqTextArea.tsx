...
import styles from './FaqTextArea.module.css';

const FaqTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FaqTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FaqTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FaqTextAreaSchema.safeParse({ value: val });
