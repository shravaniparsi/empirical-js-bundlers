...
import styles from './FaqTextArea1.module.css';

const FaqTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FaqTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FaqTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FaqTextArea1Schema.safeParse({ value: val });
