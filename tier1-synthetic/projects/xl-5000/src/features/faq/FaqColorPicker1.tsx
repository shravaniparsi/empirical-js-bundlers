...
import { z } from 'zod';
import FaqSpacer1 from './FaqSpacer1';
import FaqSkeleton from './FaqSkeleton';
import styles from './FaqColorPicker1.module.css';

const FaqColorPicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FaqColorPicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FaqColorPicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FaqColorPicker1Schema.safeParse({ value: val });
