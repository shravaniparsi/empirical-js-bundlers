...
import { z } from 'zod';
import ProjectsToggle2 from '../projects/ProjectsToggle2';
import styles from './FaqColorPicker.module.css';

const FaqColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FaqColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FaqColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FaqColorPickerSchema.safeParse({ value: val });
