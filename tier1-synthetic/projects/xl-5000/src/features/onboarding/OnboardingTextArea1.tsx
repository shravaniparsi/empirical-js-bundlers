...
import styles from './OnboardingTextArea1.module.css';

const OnboardingTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface OnboardingTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function OnboardingTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = OnboardingTextArea1Schema.safeParse({ value: val });
