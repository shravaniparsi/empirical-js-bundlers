...
import styles from './OnboardingThumbnail1.module.css';

interface OnboardingThumbnail1Item {
  priority: number;
  createdAt: Date;
  status: string;
  updatedAt: Date;
...
interface OnboardingThumbnail1Props {
  items?: OnboardingThumbnail1Item[];
...
export default function OnboardingThumbnail1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: OnboardingThumbnail1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['priority', 'createdAt', 'status']);
