...
import styles from './OnboardingSlider1.module.css';

const OnboardingSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface OnboardingSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function OnboardingSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = OnboardingSlider1Schema.safeParse({ value: val });
