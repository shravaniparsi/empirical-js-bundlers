...
import styles from './OnboardingToggle1.module.css';

const OnboardingToggle1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface OnboardingToggle1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function OnboardingToggle1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = OnboardingToggle1Schema.safeParse({ value: val });
