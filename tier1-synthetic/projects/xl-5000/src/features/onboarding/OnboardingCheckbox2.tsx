...
import styles from './OnboardingCheckbox2.module.css';

const OnboardingCheckbox2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface OnboardingCheckbox2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function OnboardingCheckbox2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = OnboardingCheckbox2Schema.safeParse({ value: val });
