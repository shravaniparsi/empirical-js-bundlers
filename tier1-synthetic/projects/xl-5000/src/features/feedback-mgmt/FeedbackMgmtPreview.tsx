...
import FeedbackMgmtScatter1 from './FeedbackMgmtScatter1';
import InventoryBanner1 from '../inventory/InventoryBanner1';
import styles from './FeedbackMgmtPreview.module.css';

interface FeedbackMgmtPreviewItem {
  description: string;
  tags: string[];
  updatedAt: Date;
  priority: number;
...
interface FeedbackMgmtPreviewProps {
  items?: FeedbackMgmtPreviewItem[];
...
export default function FeedbackMgmtPreview({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: FeedbackMgmtPreviewProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'tags', 'updatedAt']);
