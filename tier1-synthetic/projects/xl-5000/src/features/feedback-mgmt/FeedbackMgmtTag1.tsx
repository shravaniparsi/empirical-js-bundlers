...
import FeedbackMgmtTimePicker1 from './FeedbackMgmtTimePicker1';
import styles from './FeedbackMgmtTag1.module.css';

interface FeedbackMgmtTag1Item {
  updatedAt: Date;
  assignee: string;
  createdAt: Date;
  title: string;
...
interface FeedbackMgmtTag1Props {
  items?: FeedbackMgmtTag1Item[];
...
export default function FeedbackMgmtTag1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: FeedbackMgmtTag1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'assignee', 'createdAt']);
