...
import styles from './FeedbackMgmtTimePicker.module.css';

const FeedbackMgmtTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FeedbackMgmtTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FeedbackMgmtTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FeedbackMgmtTimePickerSchema.safeParse({ value: val });
