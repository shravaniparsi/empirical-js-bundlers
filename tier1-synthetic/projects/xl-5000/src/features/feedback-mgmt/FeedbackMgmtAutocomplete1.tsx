...
import styles from './FeedbackMgmtAutocomplete1.module.css';

const FeedbackMgmtAutocomplete1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FeedbackMgmtAutocomplete1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FeedbackMgmtAutocomplete1({
  label,
  name,
  ...
  onBlur,
  className,
}: FeedbackMgmtAutocomplete1Props) {
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FeedbackMgmtAutocomplete1Schema.safeParse({ value: val });
