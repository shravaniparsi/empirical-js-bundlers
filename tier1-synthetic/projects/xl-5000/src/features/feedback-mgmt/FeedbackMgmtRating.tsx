...
import { z } from 'zod';
import FeedbackMgmtModal from './FeedbackMgmtModal';
import styles from './FeedbackMgmtRating.module.css';

const FeedbackMgmtRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FeedbackMgmtRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FeedbackMgmtRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FeedbackMgmtRatingSchema.safeParse({ value: val });
