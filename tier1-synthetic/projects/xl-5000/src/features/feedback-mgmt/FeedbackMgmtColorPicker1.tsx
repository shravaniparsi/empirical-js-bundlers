...
import { z } from 'zod';
import FeedbackMgmtPrefetch3 from './FeedbackMgmtPrefetch3';
import styles from './FeedbackMgmtColorPicker1.module.css';

const FeedbackMgmtColorPicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FeedbackMgmtColorPicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FeedbackMgmtColorPicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FeedbackMgmtColorPicker1Schema.safeParse({ value: val });
