...
import { z } from 'zod';
import FeedbackMgmtAlert1 from './FeedbackMgmtAlert1';
import FeedbackMgmtFeed from './FeedbackMgmtFeed';
import SchedulingToggle1 from '../scheduling/SchedulingToggle1';
import styles from './FeedbackMgmtTimePicker1.module.css';

const FeedbackMgmtTimePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FeedbackMgmtTimePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FeedbackMgmtTimePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FeedbackMgmtTimePicker1Schema.safeParse({ value: val });
