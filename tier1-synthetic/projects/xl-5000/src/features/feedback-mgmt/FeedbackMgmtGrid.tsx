...
import styles from './FeedbackMgmtGrid.module.css';

interface FeedbackMgmtGridItem {
  status: string;
  description: string;
  tags: string[];
  createdAt: Date;
...
interface FeedbackMgmtGridProps {
  items?: FeedbackMgmtGridItem[];
...
export default function FeedbackMgmtGrid({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: FeedbackMgmtGridProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'description', 'tags']);
