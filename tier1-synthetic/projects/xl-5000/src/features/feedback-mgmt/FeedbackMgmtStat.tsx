...
import styles from './FeedbackMgmtStat.module.css';

interface FeedbackMgmtStatItem {
  createdAt: Date;
  updatedAt: Date;
  title: string;
  id: string;
...
interface FeedbackMgmtStatProps {
  items?: FeedbackMgmtStatItem[];
...
export default function FeedbackMgmtStat({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: FeedbackMgmtStatProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'updatedAt', 'title']);
