...
import { z } from 'zod';
import FeedbackMgmtTimeline from './FeedbackMgmtTimeline';
import styles from './FeedbackMgmtSelect.module.css';

const FeedbackMgmtSelectSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FeedbackMgmtSelectProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FeedbackMgmtSelect({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FeedbackMgmtSelectSchema.safeParse({ value: val });
