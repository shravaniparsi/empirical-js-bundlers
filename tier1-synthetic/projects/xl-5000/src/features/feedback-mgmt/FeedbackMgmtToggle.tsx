...
import { z } from 'zod';
import FeedbackMgmtSummary1 from './FeedbackMgmtSummary1';
import FeedbackMgmtList from './FeedbackMgmtList';
import SearchScroll1 from '../search/SearchScroll1';
import styles from './FeedbackMgmtToggle.module.css';

const FeedbackMgmtToggleSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FeedbackMgmtToggleProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FeedbackMgmtToggle({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FeedbackMgmtToggleSchema.safeParse({ value: val });
