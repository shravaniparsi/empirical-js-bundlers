...
import ShippingSpinner from '../shipping/ShippingSpinner';
import styles from './FeedbackMgmtTextArea1.module.css';

const FeedbackMgmtTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FeedbackMgmtTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FeedbackMgmtTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FeedbackMgmtTextArea1Schema.safeParse({ value: val });
