...
import { z } from 'zod';
import FeedbackMgmtLabel from './FeedbackMgmtLabel';
import FeedbackMgmtPolling from './FeedbackMgmtPolling';
import ProjectsTimePicker from '../projects/ProjectsTimePicker';
import styles from './FeedbackMgmtSearch.module.css';

const FeedbackMgmtSearchSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FeedbackMgmtSearchProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FeedbackMgmtSearch({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FeedbackMgmtSearchSchema.safeParse({ value: val });
