...
import { z } from 'zod';
import FeedbackMgmtTabs1 from './FeedbackMgmtTabs1';
import FeedbackMgmtDrawer2 from './FeedbackMgmtDrawer2';
import FeedbackMgmtSnackbar from './FeedbackMgmtSnackbar';
import styles from './FeedbackMgmtColorPicker.module.css';

const FeedbackMgmtColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface FeedbackMgmtColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function FeedbackMgmtColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = FeedbackMgmtColorPickerSchema.safeParse({ value: val });
