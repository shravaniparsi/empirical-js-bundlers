...
import MediaCheckbox1 from './MediaCheckbox1';
import MediaNotification from './MediaNotification';
import MediaPaginated2 from './MediaPaginated2';
import styles from './MediaRank.module.css';

interface MediaRankItem {
  title: string;
  updatedAt: Date;
  tags: string[];
  description: string;
...
export default function MediaRank({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'updatedAt', 'tags']);
