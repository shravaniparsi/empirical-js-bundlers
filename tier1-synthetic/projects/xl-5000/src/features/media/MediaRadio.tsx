...
import MediaComposer1 from './MediaComposer1';
import styles from './MediaRadio.module.css';

const MediaRadioSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface MediaRadioProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function MediaRadio({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = MediaRadioSchema.safeParse({ value: val });
