...
import { z } from 'zod';
import MediaPaginated1 from './MediaPaginated1';
import MediaTooltip from './MediaTooltip';
import InventorySpinner1 from '../inventory/InventorySpinner1';
import styles from './MediaCheckbox2.module.css';

const MediaCheckbox2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface MediaCheckbox2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function MediaCheckbox2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = MediaCheckbox2Schema.safeParse({ value: val });
