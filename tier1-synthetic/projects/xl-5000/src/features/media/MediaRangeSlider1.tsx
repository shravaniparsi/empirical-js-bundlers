...
import { z } from 'zod';
import FaqScatter1 from '../faq/FaqScatter1';
import styles from './MediaRangeSlider1.module.css';

const MediaRangeSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface MediaRangeSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function MediaRangeSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = MediaRangeSlider1Schema.safeParse({ value: val });
