...
import { z } from 'zod';
import MediaCheckbox2 from './MediaCheckbox2';
import MediaSparkline from './MediaSparkline';
import styles from './MediaCheckbox1.module.css';

const MediaCheckbox1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface MediaCheckbox1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function MediaCheckbox1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = MediaCheckbox1Schema.safeParse({ value: val });
