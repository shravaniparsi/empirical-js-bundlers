...
import styles from './MediaRangeSlider2.module.css';

const MediaRangeSlider2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface MediaRangeSlider2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function MediaRangeSlider2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = MediaRangeSlider2Schema.safeParse({ value: val });
