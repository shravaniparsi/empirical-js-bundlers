...
import MediaLink1 from './MediaLink1';
...
import styles from './MediaRank1.module.css';

interface MediaRank1Item {
  priority: number;
  assignee: string;
  tags: string[];
  id: string;
...
interface MediaRank1Props {
  items?: MediaRank1Item[];
...
export default function MediaRank1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['priority', 'assignee', 'tags']);
