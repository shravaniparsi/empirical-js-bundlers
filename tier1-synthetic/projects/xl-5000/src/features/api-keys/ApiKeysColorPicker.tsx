...
import ApiKeysBarChart1 from './ApiKeysBarChart1';
import styles from './ApiKeysColorPicker.module.css';

const ApiKeysColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ApiKeysColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ApiKeysColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ApiKeysColorPickerSchema.safeParse({ value: val });
