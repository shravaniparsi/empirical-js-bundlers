...
import ApiKeysBottomNav from './ApiKeysBottomNav';
import ReportsSpinner1 from '../reports/ReportsSpinner1';
import styles from './ApiKeysThumbnail1.module.css';

interface ApiKeysThumbnail1Item {
  assignee: string;
  status: string;
  updatedAt: Date;
  tags: string[];
...
interface ApiKeysThumbnail1Props {
  items?: ApiKeysThumbnail1Item[];
...
export default function ApiKeysThumbnail1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: ApiKeysThumbnail1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'status', 'updatedAt']);
