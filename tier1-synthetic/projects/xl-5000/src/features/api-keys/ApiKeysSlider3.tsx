...
import ApiKeysResponsive1 from './ApiKeysResponsive1';
import InventoryAreaChart from '../inventory/InventoryAreaChart';
import styles from './ApiKeysSlider3.module.css';

const ApiKeysSlider3Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ApiKeysSlider3Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ApiKeysSlider3({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ApiKeysSlider3Schema.safeParse({ value: val });
