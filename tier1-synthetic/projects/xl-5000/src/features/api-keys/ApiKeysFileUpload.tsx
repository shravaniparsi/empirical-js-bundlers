...
import { z } from 'zod';
import ApiKeysPopover1 from './ApiKeysPopover1';
import ApiKeysPaginated from './ApiKeysPaginated';
import RolesProgress1 from '../roles/RolesProgress1';
import ApiKeysNotification1 from './ApiKeysNotification1';
import styles from './ApiKeysFileUpload.module.css';

const ApiKeysFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ApiKeysFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ApiKeysFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ApiKeysFileUploadSchema.safeParse({ value: val });
