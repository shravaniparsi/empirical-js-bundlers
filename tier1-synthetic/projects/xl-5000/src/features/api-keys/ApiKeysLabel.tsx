...
import styles from './ApiKeysLabel.module.css';

interface ApiKeysLabelItem {
  status: string;
  priority: number;
  tags: string[];
  createdAt: Date;
...
interface ApiKeysLabelProps {
  items?: ApiKeysLabelItem[];
...
export default function ApiKeysLabel({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'priority', 'tags']);
