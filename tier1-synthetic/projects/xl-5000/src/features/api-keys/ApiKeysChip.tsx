...
import ApiKeysBottomNav1 from './ApiKeysBottomNav1';
import ApiKeysInfiniteScroll2 from './ApiKeysInfiniteScroll2';
import ApiKeysCard1 from './ApiKeysCard1';
import DashboardPanel1 from '../dashboard/DashboardPanel1';
import InventoryProgress from '../inventory/InventoryProgress';
import styles from './ApiKeysChip.module.css';

interface ApiKeysChipItem {
  createdAt: Date;
  tags: string[];
  priority: number;
  id: string;
...
interface ApiKeysChipProps {
  items?: ApiKeysChipItem[];
...
export default function ApiKeysChip({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'tags', 'priority']);
