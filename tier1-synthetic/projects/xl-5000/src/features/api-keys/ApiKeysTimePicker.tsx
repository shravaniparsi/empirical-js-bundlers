...
import { z } from 'zod';
import ApiKeysRank2 from './ApiKeysRank2';
import ApiKeysColorPicker from './ApiKeysColorPicker';
import styles from './ApiKeysTimePicker.module.css';

const ApiKeysTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ApiKeysTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ApiKeysTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ApiKeysTimePickerSchema.safeParse({ value: val });
