...
import ApiKeysResponsive from './ApiKeysResponsive';
import styles from './ApiKeysTextArea.module.css';

const ApiKeysTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ApiKeysTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ApiKeysTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ApiKeysTextAreaSchema.safeParse({ value: val });
