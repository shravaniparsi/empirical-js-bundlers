...
import PermissionsMenu from '../permissions/PermissionsMenu';
import styles from './ApiKeysThumbnail2.module.css';

interface ApiKeysThumbnail2Item {
  status: string;
  title: string;
  createdAt: Date;
  id: string;
...
interface ApiKeysThumbnail2Props {
  items?: ApiKeysThumbnail2Item[];
...
export default function ApiKeysThumbnail2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: ApiKeysThumbnail2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'title', 'createdAt']);
