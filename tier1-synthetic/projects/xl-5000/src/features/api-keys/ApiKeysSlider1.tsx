...
import ApiKeysScroll from './ApiKeysScroll';
import TemplatesStack1 from '../templates/TemplatesStack1';
import FeedbackMgmtSummary from '../feedback-mgmt/FeedbackMgmtSummary';
import styles from './ApiKeysSlider1.module.css';

const ApiKeysSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ApiKeysSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ApiKeysSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ApiKeysSlider1Schema.safeParse({ value: val });
