...
import ApiKeysRank1 from './ApiKeysRank1';
import ApiKeysToast1 from './ApiKeysToast1';
import ReportsMeter1 from '../reports/ReportsMeter1';
import styles from './ApiKeysScore1.module.css';

interface ApiKeysScore1Item {
  status: string;
  assignee: string;
  tags: string[];
  updatedAt: Date;
...
interface ApiKeysScore1Props {
  items?: ApiKeysScore1Item[];
...
export default function ApiKeysScore1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'assignee', 'tags']);
