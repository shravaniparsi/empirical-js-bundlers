...
import ApiKeysTextArea from './ApiKeysTextArea';
import styles from './ApiKeysTimeline.module.css';

interface ApiKeysTimelineItem {
  description: string;
  status: string;
  priority: number;
  id: string;
...
interface ApiKeysTimelineProps {
  items?: ApiKeysTimelineItem[];
...
export default function ApiKeysTimeline({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: ApiKeysTimelineProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'status', 'priority']);
