...
import { z } from 'zod';
import ApiKeysScroll1 from './ApiKeysScroll1';
import ApiKeysBreadcrumb1 from './ApiKeysBreadcrumb1';
import ReviewsSpacer1 from '../reviews/ReviewsSpacer1';
import styles from './ApiKeysDatePicker.module.css';

const ApiKeysDatePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ApiKeysDatePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ApiKeysDatePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ApiKeysDatePickerSchema.safeParse({ value: val });
