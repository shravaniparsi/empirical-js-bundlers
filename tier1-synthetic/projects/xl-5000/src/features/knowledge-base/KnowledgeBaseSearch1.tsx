...
import styles from './KnowledgeBaseSearch1.module.css';

const KnowledgeBaseSearch1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface KnowledgeBaseSearch1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function KnowledgeBaseSearch1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = KnowledgeBaseSearch1Schema.safeParse({ value: val });
