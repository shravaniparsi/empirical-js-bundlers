...
import { z } from 'zod';
import KnowledgeBaseScroll from './KnowledgeBaseScroll';
import KnowledgeBaseResponsive from './KnowledgeBaseResponsive';
import styles from './KnowledgeBaseTextArea.module.css';

const KnowledgeBaseTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface KnowledgeBaseTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function KnowledgeBaseTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = KnowledgeBaseTextAreaSchema.safeParse({ value: val });
