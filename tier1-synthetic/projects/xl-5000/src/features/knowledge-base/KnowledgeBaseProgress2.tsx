...
import KnowledgeBaseRank1 from './KnowledgeBaseRank1';
import ReportsMeter1 from '../reports/ReportsMeter1';
import MediaBadge from '../media/MediaBadge';
import styles from './KnowledgeBaseProgress2.module.css';

interface KnowledgeBaseProgress2Item {
  description: string;
  createdAt: Date;
  title: string;
  id: string;
...
interface KnowledgeBaseProgress2Props {
  items?: KnowledgeBaseProgress2Item[];
...
export default function KnowledgeBaseProgress2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: KnowledgeBaseProgress2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'createdAt', 'title']);
