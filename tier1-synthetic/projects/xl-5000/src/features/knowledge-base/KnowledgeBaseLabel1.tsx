...
import styles from './KnowledgeBaseLabel1.module.css';

interface KnowledgeBaseLabel1Item {
  priority: number;
  assignee: string;
  title: string;
  createdAt: Date;
...
interface KnowledgeBaseLabel1Props {
  items?: KnowledgeBaseLabel1Item[];
...
export default function KnowledgeBaseLabel1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: KnowledgeBaseLabel1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['priority', 'assignee', 'title']);
