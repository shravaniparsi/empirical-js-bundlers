...
import { z } from 'zod';
import KnowledgeBaseProgress from './KnowledgeBaseProgress';
import KnowledgeBaseProvider from './KnowledgeBaseProvider';
import styles from './KnowledgeBaseRangeSlider.module.css';

const KnowledgeBaseRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface KnowledgeBaseRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function KnowledgeBaseRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = KnowledgeBaseRangeSliderSchema.safeParse({ value: val });
