...
import styles from './KnowledgeBaseFeed1.module.css';

interface KnowledgeBaseFeed1Item {
  createdAt: Date;
  description: string;
  priority: number;
  id: string;
...
interface KnowledgeBaseFeed1Props {
  items?: KnowledgeBaseFeed1Item[];
...
export default function KnowledgeBaseFeed1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: KnowledgeBaseFeed1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'description', 'priority']);
