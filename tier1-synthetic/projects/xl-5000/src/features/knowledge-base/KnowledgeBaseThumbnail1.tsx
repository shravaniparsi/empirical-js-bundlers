...
import KnowledgeBasePlaceholder1 from './KnowledgeBasePlaceholder1';
import KnowledgeBaseToast1 from './KnowledgeBaseToast1';
import KnowledgeBaseRating from './KnowledgeBaseRating';
import styles from './KnowledgeBaseThumbnail1.module.css';

interface KnowledgeBaseThumbnail1Item {
  assignee: string;
  updatedAt: Date;
  description: string;
  status: string;
...
interface KnowledgeBaseThumbnail1Props {
  items?: KnowledgeBaseThumbnail1Item[];
...
export default function KnowledgeBaseThumbnail1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: KnowledgeBaseThumbnail1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'updatedAt', 'description']);
