...
import KnowledgeBasePanel1 from './KnowledgeBasePanel1';
import OrdersHeatmap1 from '../orders/OrdersHeatmap1';
import styles from './KnowledgeBaseProgress1.module.css';

interface KnowledgeBaseProgress1Item {
  updatedAt: Date;
  assignee: string;
  priority: number;
  description: string;
...
interface KnowledgeBaseProgress1Props {
  items?: KnowledgeBaseProgress1Item[];
...
export default function KnowledgeBaseProgress1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: KnowledgeBaseProgress1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'assignee', 'priority']);
