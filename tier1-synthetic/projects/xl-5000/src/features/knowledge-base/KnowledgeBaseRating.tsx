...
import { z } from 'zod';
import KnowledgeBaseRank from './KnowledgeBaseRank';
import KnowledgeBaseSummary1 from './KnowledgeBaseSummary1';
import OrdersHeatmap1 from '../orders/OrdersHeatmap1';
import styles from './KnowledgeBaseRating.module.css';

const KnowledgeBaseRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface KnowledgeBaseRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function KnowledgeBaseRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = KnowledgeBaseRatingSchema.safeParse({ value: val });
