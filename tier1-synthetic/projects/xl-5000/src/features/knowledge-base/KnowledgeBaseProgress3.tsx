...
import KnowledgeBaseResponsive from './KnowledgeBaseResponsive';
import KnowledgeBaseLabel1 from './KnowledgeBaseLabel1';
import styles from './KnowledgeBaseProgress3.module.css';

interface KnowledgeBaseProgress3Item {
  title: string;
  createdAt: Date;
  status: string;
  tags: string[];
...
interface KnowledgeBaseProgress3Props {
  items?: KnowledgeBaseProgress3Item[];
...
export default function KnowledgeBaseProgress3({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: KnowledgeBaseProgress3Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'createdAt', 'status']);
