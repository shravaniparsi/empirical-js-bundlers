...
import { z } from 'zod';
import KnowledgeBaseStepper1 from './KnowledgeBaseStepper1';
import ShippingTag1 from '../shipping/ShippingTag1';
import styles from './KnowledgeBaseToggle1.module.css';

const KnowledgeBaseToggle1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface KnowledgeBaseToggle1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function KnowledgeBaseToggle1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = KnowledgeBaseToggle1Schema.safeParse({ value: val });
