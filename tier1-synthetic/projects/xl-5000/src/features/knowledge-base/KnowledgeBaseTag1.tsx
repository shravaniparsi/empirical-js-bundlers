...
import KnowledgeBaseChip2 from './KnowledgeBaseChip2';
import KnowledgeBaseFeed from './KnowledgeBaseFeed';
import KnowledgeBasePolling1 from './KnowledgeBasePolling1';
import styles from './KnowledgeBaseTag1.module.css';

interface KnowledgeBaseTag1Item {
  status: string;
  createdAt: Date;
  updatedAt: Date;
  priority: number;
...
interface KnowledgeBaseTag1Props {
  items?: KnowledgeBaseTag1Item[];
...
export default function KnowledgeBaseTag1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: KnowledgeBaseTag1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'createdAt', 'updatedAt']);
