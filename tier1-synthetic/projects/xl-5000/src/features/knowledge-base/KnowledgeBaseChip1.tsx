...
import KnowledgeBaseScatter from './KnowledgeBaseScatter';
import KnowledgeBaseSplit1 from './KnowledgeBaseSplit1';
import KnowledgeBaseProvider from './KnowledgeBaseProvider';
import styles from './KnowledgeBaseChip1.module.css';

interface KnowledgeBaseChip1Item {
  updatedAt: Date;
  description: string;
  title: string;
  tags: string[];
...
interface KnowledgeBaseChip1Props {
  items?: KnowledgeBaseChip1Item[];
...
export default function KnowledgeBaseChip1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: KnowledgeBaseChip1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'description', 'title']);
