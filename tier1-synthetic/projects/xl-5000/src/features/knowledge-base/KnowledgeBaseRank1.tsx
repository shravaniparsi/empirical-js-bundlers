...
import KnowledgeBaseSkeleton1 from './KnowledgeBaseSkeleton1';
...
import styles from './KnowledgeBaseRank1.module.css';

interface KnowledgeBaseRank1Item {
  title: string;
  priority: number;
  description: string;
  id: string;
...
interface KnowledgeBaseRank1Props {
  items?: KnowledgeBaseRank1Item[];
...
export default function KnowledgeBaseRank1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: KnowledgeBaseRank1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'priority', 'description']);
