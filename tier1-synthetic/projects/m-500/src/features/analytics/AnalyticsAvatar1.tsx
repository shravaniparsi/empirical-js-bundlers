...
import styles from './AnalyticsAvatar1.module.css';

interface AnalyticsAvatar1Item {
  status: string;
  priority: number;
  description: string;
  title: string;
...
interface AnalyticsAvatar1Props {
  items?: AnalyticsAvatar1Item[];
...
export default function AnalyticsAvatar1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: AnalyticsAvatar1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'priority', 'description']);
