...
import { z } from 'zod';
import AnalyticsTreeView from './AnalyticsTreeView';
import AnalyticsSticky1 from './AnalyticsSticky1';
import styles from './AnalyticsColorPicker.module.css';

const AnalyticsColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AnalyticsColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AnalyticsColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AnalyticsColorPickerSchema.safeParse({ value: val });
