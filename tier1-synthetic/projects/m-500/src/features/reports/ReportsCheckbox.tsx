...
import styles from './ReportsCheckbox.module.css';

const ReportsCheckboxSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReportsCheckboxProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReportsCheckbox({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReportsCheckboxSchema.safeParse({ value: val });
