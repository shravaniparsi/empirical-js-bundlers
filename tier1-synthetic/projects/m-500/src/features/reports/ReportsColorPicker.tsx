...
import { z } from 'zod';
import ShippingDivider1 from '../shipping/ShippingDivider1';
import ProductsAlert from '../products/ProductsAlert';
import styles from './ReportsColorPicker.module.css';

const ReportsColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReportsColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReportsColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReportsColorPickerSchema.safeParse({ value: val });
