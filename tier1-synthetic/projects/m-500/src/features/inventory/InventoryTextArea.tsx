...
import styles from './InventoryTextArea.module.css';

const InventoryTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface InventoryTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function InventoryTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = InventoryTextAreaSchema.safeParse({ value: val });
