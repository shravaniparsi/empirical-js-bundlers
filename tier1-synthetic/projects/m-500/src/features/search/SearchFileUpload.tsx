...
import styles from './SearchFileUpload.module.css';

const SearchFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SearchFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SearchFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SearchFileUploadSchema.safeParse({ value: val });
