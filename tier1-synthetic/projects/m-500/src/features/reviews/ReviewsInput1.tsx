...
import styles from './ReviewsInput1.module.css';

const ReviewsInput1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsInput1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsInput1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsInput1Schema.safeParse({ value: val });
