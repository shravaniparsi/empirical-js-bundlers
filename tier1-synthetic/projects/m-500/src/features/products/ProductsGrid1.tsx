...
import ProductsCollapse1 from './ProductsCollapse1';
import ProductsMenu1 from './ProductsMenu1';
import ProductsAccordion from './ProductsAccordion';
import styles from './ProductsGrid1.module.css';

interface ProductsGrid1Item {
  priority: number;
  assignee: string;
  status: string;
  tags: string[];
...
export default function ProductsGrid1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['priority', 'assignee', 'status']);
