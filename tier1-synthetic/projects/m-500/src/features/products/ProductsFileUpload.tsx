...
import { z } from 'zod';
import ProductsPrefetch from './ProductsPrefetch';
import styles from './ProductsFileUpload.module.css';

const ProductsFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProductsFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProductsFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProductsFileUploadSchema.safeParse({ value: val });
