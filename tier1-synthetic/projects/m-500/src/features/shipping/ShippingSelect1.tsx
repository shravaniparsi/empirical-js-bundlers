...
import { z } from 'zod';
import ShippingFileUpload from './ShippingFileUpload';
import ShippingProgress1 from './ShippingProgress1';
import InventoryBarChart from '../inventory/InventoryBarChart';
import styles from './ShippingSelect1.module.css';

const ShippingSelect1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ShippingSelect1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ShippingSelect1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ShippingSelect1Schema.safeParse({ value: val });
