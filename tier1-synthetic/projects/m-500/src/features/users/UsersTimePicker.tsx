...
import { z } from 'zod';
import UsersSticky from './UsersSticky';
import styles from './UsersTimePicker.module.css';

const UsersTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface UsersTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function UsersTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = UsersTimePickerSchema.safeParse({ value: val });
