...
import styles from './UsersRangeSlider1.module.css';

const UsersRangeSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface UsersRangeSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function UsersRangeSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = UsersRangeSlider1Schema.safeParse({ value: val });
