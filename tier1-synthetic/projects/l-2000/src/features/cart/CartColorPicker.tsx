...
import { z } from 'zod';
import CartBridge1 from './CartBridge1';
import CartStat from './CartStat';
import CartTable1 from './CartTable1';
import styles from './CartColorPicker.module.css';

const CartColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CartColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CartColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CartColorPickerSchema.safeParse({ value: val });
