...
import styles from './CatalogGrid2.module.css';

interface CatalogGrid2Item {
  status: string;
  title: string;
  description: string;
  createdAt: Date;
...
interface CatalogGrid2Props {
  items?: CatalogGrid2Item[];
...
export default function CatalogGrid2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'title', 'description']);
