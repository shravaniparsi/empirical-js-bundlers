...
import CatalogPrefetch from './CatalogPrefetch';
import styles from './CatalogPreview1.module.css';

interface CatalogPreview1Item {
  description: string;
  priority: number;
  assignee: string;
  updatedAt: Date;
...
interface CatalogPreview1Props {
  items?: CatalogPreview1Item[];
...
export default function CatalogPreview1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: CatalogPreview1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'priority', 'assignee']);
