...
import styles from './CatalogProgress1.module.css';

interface CatalogProgress1Item {
  createdAt: Date;
  assignee: string;
  status: string;
  tags: string[];
...
interface CatalogProgress1Props {
  items?: CatalogProgress1Item[];
...
export default function CatalogProgress1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: CatalogProgress1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'assignee', 'status']);
