...
import { z } from 'zod';
import ReportsSnackbar from './ReportsSnackbar';
import ReportsSummary1 from './ReportsSummary1';
import styles from './ReportsFileUpload.module.css';

const ReportsFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReportsFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReportsFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReportsFileUploadSchema.safeParse({ value: val });
