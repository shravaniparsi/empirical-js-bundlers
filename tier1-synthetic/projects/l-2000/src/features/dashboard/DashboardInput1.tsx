...
import styles from './DashboardInput1.module.css';

const DashboardInput1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DashboardInput1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DashboardInput1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DashboardInput1Schema.safeParse({ value: val });
