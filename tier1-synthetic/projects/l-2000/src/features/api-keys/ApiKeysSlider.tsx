...
import { z } from 'zod';
import ApiKeysProgress from './ApiKeysProgress';
import ApiKeysStepper from './ApiKeysStepper';
import ApiKeysInfiniteScroll from './ApiKeysInfiniteScroll';
import styles from './ApiKeysSlider.module.css';

const ApiKeysSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ApiKeysSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ApiKeysSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ApiKeysSliderSchema.safeParse({ value: val });
