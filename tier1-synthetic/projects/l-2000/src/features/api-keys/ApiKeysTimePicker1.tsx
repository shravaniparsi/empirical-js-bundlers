...
import { z } from 'zod';
import ApiKeysMenu1 from './ApiKeysMenu1';
import PaymentsTextArea from '../payments/PaymentsTextArea';
import styles from './ApiKeysTimePicker1.module.css';

const ApiKeysTimePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ApiKeysTimePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ApiKeysTimePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ApiKeysTimePicker1Schema.safeParse({ value: val });
