...
import { z } from 'zod';
import ApiKeysTooltip1 from './ApiKeysTooltip1';
import ApiKeysTooltip from './ApiKeysTooltip';
import ProfileLoader1 from '../profile/ProfileLoader1';
import styles from './ApiKeysAutocomplete.module.css';

const ApiKeysAutocompleteSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ApiKeysAutocompleteProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ApiKeysAutocomplete({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ApiKeysAutocompleteSchema.safeParse({ value: val });
