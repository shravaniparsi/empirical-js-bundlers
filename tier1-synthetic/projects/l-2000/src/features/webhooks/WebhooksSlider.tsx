...
import { z } from 'zod';
import WebhooksTooltip from './WebhooksTooltip';
import WebhooksSpinner from './WebhooksSpinner';
import styles from './WebhooksSlider.module.css';

const WebhooksSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WebhooksSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WebhooksSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WebhooksSliderSchema.safeParse({ value: val });
