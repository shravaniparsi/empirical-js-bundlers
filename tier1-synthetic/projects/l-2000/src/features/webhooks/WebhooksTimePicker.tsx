...
import styles from './WebhooksTimePicker.module.css';

const WebhooksTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WebhooksTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WebhooksTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WebhooksTimePickerSchema.safeParse({ value: val });
