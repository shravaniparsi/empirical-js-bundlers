...
import { z } from 'zod';
import WebhooksPlaceholder1 from './WebhooksPlaceholder1';
import WebhooksSparkline from './WebhooksSparkline';
import styles from './WebhooksRating.module.css';

const WebhooksRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WebhooksRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WebhooksRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WebhooksRatingSchema.safeParse({ value: val });
