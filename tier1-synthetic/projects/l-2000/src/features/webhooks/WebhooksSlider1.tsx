...
import { z } from 'zod';
import WebhooksComposer from './WebhooksComposer';
import WebhooksLoader from './WebhooksLoader';
import styles from './WebhooksSlider1.module.css';

const WebhooksSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WebhooksSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WebhooksSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WebhooksSlider1Schema.safeParse({ value: val });
