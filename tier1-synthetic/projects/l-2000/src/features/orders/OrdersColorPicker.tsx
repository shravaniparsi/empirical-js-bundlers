...
import { z } from 'zod';
import OrdersModal2 from './OrdersModal2';
import OrdersModal from './OrdersModal';
import styles from './OrdersColorPicker.module.css';

const OrdersColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface OrdersColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function OrdersColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = OrdersColorPickerSchema.safeParse({ value: val });
