...
import { z } from 'zod';
import InventoryTimePicker from './InventoryTimePicker';
import InventoryList1 from './InventoryList1';
import styles from './InventoryToggle2.module.css';

const InventoryToggle2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface InventoryToggle2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function InventoryToggle2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = InventoryToggle2Schema.safeParse({ value: val });
