...
import CartTable1 from '../cart/CartTable1';
import styles from './InventoryToggle1.module.css';

const InventoryToggle1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface InventoryToggle1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function InventoryToggle1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = InventoryToggle1Schema.safeParse({ value: val });
