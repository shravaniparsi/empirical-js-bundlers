...
import styles from './InventoryTimePicker.module.css';

const InventoryTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface InventoryTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function InventoryTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = InventoryTimePickerSchema.safeParse({ value: val });
