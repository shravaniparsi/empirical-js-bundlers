...
import AnalyticsPlaceholder1 from './AnalyticsPlaceholder1';
import styles from './AnalyticsRadio1.module.css';

const AnalyticsRadio1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AnalyticsRadio1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AnalyticsRadio1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AnalyticsRadio1Schema.safeParse({ value: val });
