...
import WebhooksRating from '../webhooks/WebhooksRating';
import styles from './AnalyticsRating1.module.css';

const AnalyticsRating1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AnalyticsRating1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AnalyticsRating1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AnalyticsRating1Schema.safeParse({ value: val });
