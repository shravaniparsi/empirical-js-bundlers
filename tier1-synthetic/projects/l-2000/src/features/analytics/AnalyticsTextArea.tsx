...
import { z } from 'zod';
import AnalyticsLoader1 from './AnalyticsLoader1';
import ReportsRadio from '../reports/ReportsRadio';
import styles from './AnalyticsTextArea.module.css';

const AnalyticsTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AnalyticsTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AnalyticsTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AnalyticsTextAreaSchema.safeParse({ value: val });
