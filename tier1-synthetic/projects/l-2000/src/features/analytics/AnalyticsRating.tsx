...
import IntegrationsChip1 from '../integrations/IntegrationsChip1';
import styles from './AnalyticsRating.module.css';

const AnalyticsRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AnalyticsRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AnalyticsRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AnalyticsRatingSchema.safeParse({ value: val });
