...
import WorkflowsStack1 from './WorkflowsStack1';
import WorkflowsBadge from './WorkflowsBadge';
import styles from './WorkflowsTimePicker.module.css';

const WorkflowsTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsTimePickerSchema.safeParse({ value: val });
