...
import WorkflowsColorPicker1 from './WorkflowsColorPicker1';
import styles from './WorkflowsSlider.module.css';

const WorkflowsSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsSlider({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsSliderSchema.safeParse({ value: val });
