...
import styles from './WorkflowsRangeSlider1.module.css';

const WorkflowsRangeSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsRangeSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsRangeSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsRangeSlider1Schema.safeParse({ value: val });
