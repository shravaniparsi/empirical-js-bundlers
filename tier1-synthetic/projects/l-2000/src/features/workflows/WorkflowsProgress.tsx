...
import WorkflowsRadio from './WorkflowsRadio';
import WorkflowsBadge1 from './WorkflowsBadge1';
import styles from './WorkflowsProgress.module.css';

interface WorkflowsProgressItem {
  createdAt: Date;
  tags: string[];
  title: string;
  assignee: string;
...
interface WorkflowsProgressProps {
  items?: WorkflowsProgressItem[];
...
export default function WorkflowsProgress({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: WorkflowsProgressProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'tags', 'title']);
