...
import { z } from 'zod';
import WorkflowsSlider from './WorkflowsSlider';
import WorkflowsHeatmap1 from './WorkflowsHeatmap1';
import styles from './WorkflowsDatePicker.module.css';

const WorkflowsDatePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsDatePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsDatePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsDatePickerSchema.safeParse({ value: val });
