...
import { z } from 'zod';
import WorkflowsSnackbar1 from './WorkflowsSnackbar1';
import WorkflowsFeed1 from './WorkflowsFeed1';
import WorkflowsBarChart from './WorkflowsBarChart';
import styles from './WorkflowsInput.module.css';

const WorkflowsInputSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface WorkflowsInputProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function WorkflowsInput({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = WorkflowsInputSchema.safeParse({ value: val });
