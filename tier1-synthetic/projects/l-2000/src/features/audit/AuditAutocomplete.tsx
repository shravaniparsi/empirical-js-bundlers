...
import { z } from 'zod';
import AuditConfirm1 from './AuditConfirm1';
import styles from './AuditAutocomplete.module.css';

const AuditAutocompleteSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AuditAutocompleteProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AuditAutocomplete({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AuditAutocompleteSchema.safeParse({ value: val });
