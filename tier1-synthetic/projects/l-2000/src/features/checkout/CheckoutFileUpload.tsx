...
import CheckoutToggle1 from './CheckoutToggle1';
import WebhooksPanel from '../webhooks/WebhooksPanel';
import styles from './CheckoutFileUpload.module.css';

const CheckoutFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CheckoutFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CheckoutFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CheckoutFileUploadSchema.safeParse({ value: val });
