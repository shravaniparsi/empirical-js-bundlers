...
import styles from './CheckoutRating.module.css';

const CheckoutRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CheckoutRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CheckoutRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CheckoutRatingSchema.safeParse({ value: val });
