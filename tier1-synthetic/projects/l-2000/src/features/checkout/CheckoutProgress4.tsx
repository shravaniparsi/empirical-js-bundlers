...
import styles from './CheckoutProgress4.module.css';

interface CheckoutProgress4Item {
  tags: string[];
  createdAt: Date;
  assignee: string;
  priority: number;
...
interface CheckoutProgress4Props {
  items?: CheckoutProgress4Item[];
...
export default function CheckoutProgress4({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: CheckoutProgress4Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'createdAt', 'assignee']);
