...
import { z } from 'zod';
import UsersColorPicker from './UsersColorPicker';
...
import styles from './UsersTextArea.module.css';

const UsersTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface UsersTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function UsersTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = UsersTextAreaSchema.safeParse({ value: val });
