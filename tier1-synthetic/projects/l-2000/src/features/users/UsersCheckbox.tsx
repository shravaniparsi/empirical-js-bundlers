...
import { z } from 'zod';
import UsersConfirm1 from './UsersConfirm1';
import UsersComposer from './UsersComposer';
import styles from './UsersCheckbox.module.css';

const UsersCheckboxSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface UsersCheckboxProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function UsersCheckbox({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = UsersCheckboxSchema.safeParse({ value: val });
