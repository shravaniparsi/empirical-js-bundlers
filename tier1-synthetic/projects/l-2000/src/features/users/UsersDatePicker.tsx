...
import styles from './UsersDatePicker.module.css';

const UsersDatePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface UsersDatePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function UsersDatePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = UsersDatePickerSchema.safeParse({ value: val });
