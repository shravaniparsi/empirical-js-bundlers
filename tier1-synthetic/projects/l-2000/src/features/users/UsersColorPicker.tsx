...
import { z } from 'zod';
import UsersTabs1 from './UsersTabs1';
import WebhooksSlider from '../webhooks/WebhooksSlider';
import styles from './UsersColorPicker.module.css';

const UsersColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface UsersColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function UsersColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = UsersColorPickerSchema.safeParse({ value: val });
