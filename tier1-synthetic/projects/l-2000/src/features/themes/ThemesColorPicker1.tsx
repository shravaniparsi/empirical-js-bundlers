...
import { z } from 'zod';
import ThemesLabel from './ThemesLabel';
import ThemesBottomNav1 from './ThemesBottomNav1';
import ThemesTextArea from './ThemesTextArea';
import styles from './ThemesColorPicker1.module.css';

const ThemesColorPicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ThemesColorPicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ThemesColorPicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ThemesColorPicker1Schema.safeParse({ value: val });
