...
import { z } from 'zod';
import ThemesNotification from './ThemesNotification';
import styles from './ThemesColorPicker2.module.css';

const ThemesColorPicker2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ThemesColorPicker2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ThemesColorPicker2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ThemesColorPicker2Schema.safeParse({ value: val });
