...
import styles from './ThemesCheckbox1.module.css';

const ThemesCheckbox1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ThemesCheckbox1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ThemesCheckbox1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ThemesCheckbox1Schema.safeParse({ value: val });
