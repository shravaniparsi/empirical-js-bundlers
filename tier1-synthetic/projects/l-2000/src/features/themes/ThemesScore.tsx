...
import ThemesComposer from './ThemesComposer';
import ThemesHeader1 from './ThemesHeader1';
import ShippingMeter from '../shipping/ShippingMeter';
import CartSpinner from '../cart/CartSpinner';
import styles from './ThemesScore.module.css';

interface ThemesScoreItem {
  assignee: string;
  description: string;
  priority: number;
  title: string;
...
interface ThemesScoreProps {
  items?: ThemesScoreItem[];
...
export default function ThemesScore({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'description', 'priority']);
