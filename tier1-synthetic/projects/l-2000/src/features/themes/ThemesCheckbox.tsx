...
import styles from './ThemesCheckbox.module.css';

const ThemesCheckboxSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ThemesCheckboxProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ThemesCheckbox({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ThemesCheckboxSchema.safeParse({ value: val });
