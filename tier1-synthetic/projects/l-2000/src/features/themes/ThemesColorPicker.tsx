...
import ThemesTabs from './ThemesTabs';
import AdminMeter from '../admin/AdminMeter';
import styles from './ThemesColorPicker.module.css';

const ThemesColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ThemesColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ThemesColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ThemesColorPickerSchema.safeParse({ value: val });
