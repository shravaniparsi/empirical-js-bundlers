...
import styles from './ProfileRangeSlider1.module.css';

const ProfileRangeSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProfileRangeSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProfileRangeSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProfileRangeSlider1Schema.safeParse({ value: val });
