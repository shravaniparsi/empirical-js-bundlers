...
import { z } from 'zod';
import ProfilePolling from './ProfilePolling';
import ProfileScatter2 from './ProfileScatter2';
import styles from './ProfileAutocomplete1.module.css';

const ProfileAutocomplete1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProfileAutocomplete1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProfileAutocomplete1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProfileAutocomplete1Schema.safeParse({ value: val });
