...
import { z } from 'zod';
import ShippingHeader1 from './ShippingHeader1';
import ShippingTabs from './ShippingTabs';
import styles from './ShippingTextArea1.module.css';

const ShippingTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ShippingTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ShippingTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ShippingTextArea1Schema.safeParse({ value: val });
