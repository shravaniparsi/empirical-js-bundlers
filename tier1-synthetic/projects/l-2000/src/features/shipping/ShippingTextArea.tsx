...
import ShippingBanner1 from './ShippingBanner1';
import styles from './ShippingTextArea.module.css';

const ShippingTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ShippingTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ShippingTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ShippingTextAreaSchema.safeParse({ value: val });
