...
import { z } from 'zod';
import DocumentsDatePicker from './DocumentsDatePicker';
...
import styles from './DocumentsAutocomplete.module.css';

const DocumentsAutocompleteSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DocumentsAutocompleteProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DocumentsAutocomplete({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DocumentsAutocompleteSchema.safeParse({ value: val });
