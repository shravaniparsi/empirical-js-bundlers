...
import { z } from 'zod';
import DocumentsSticky1 from './DocumentsSticky1';
import ProfileLoader1 from '../profile/ProfileLoader1';
import styles from './DocumentsFileUpload1.module.css';

const DocumentsFileUpload1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DocumentsFileUpload1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DocumentsFileUpload1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DocumentsFileUpload1Schema.safeParse({ value: val });
