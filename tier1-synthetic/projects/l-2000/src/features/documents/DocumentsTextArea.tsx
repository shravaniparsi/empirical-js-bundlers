...
import { z } from 'zod';
import DocumentsRadio from './DocumentsRadio';
import DocumentsAutocomplete from './DocumentsAutocomplete';
import DocumentsRetry1 from './DocumentsRetry1';
import ShippingMeter from '../shipping/ShippingMeter';
import styles from './DocumentsTextArea.module.css';

const DocumentsTextAreaSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DocumentsTextAreaProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DocumentsTextArea({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DocumentsTextAreaSchema.safeParse({ value: val });
