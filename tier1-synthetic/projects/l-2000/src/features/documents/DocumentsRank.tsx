...
import DocumentsDonut from './DocumentsDonut';
import DocumentsFileUpload1 from './DocumentsFileUpload1';
import DocumentsCheckbox from './DocumentsCheckbox';
import styles from './DocumentsRank.module.css';

interface DocumentsRankItem {
  title: string;
  status: string;
  description: string;
  updatedAt: Date;
...
interface DocumentsRankProps {
  items?: DocumentsRankItem[];
...
export default function DocumentsRank({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'status', 'description']);
