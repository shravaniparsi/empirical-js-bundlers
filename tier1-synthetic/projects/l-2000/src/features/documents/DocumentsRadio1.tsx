...
import styles from './DocumentsRadio1.module.css';

const DocumentsRadio1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DocumentsRadio1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DocumentsRadio1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DocumentsRadio1Schema.safeParse({ value: val });
