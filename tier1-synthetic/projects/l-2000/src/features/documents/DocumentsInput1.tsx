...
import DocumentsGauge1 from './DocumentsGauge1';
import DocumentsRank from './DocumentsRank';
import styles from './DocumentsInput1.module.css';

const DocumentsInput1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface DocumentsInput1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function DocumentsInput1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = DocumentsInput1Schema.safeParse({ value: val });
