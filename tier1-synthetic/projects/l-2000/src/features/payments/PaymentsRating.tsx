...
import { z } from 'zod';
import PaymentsAvatar1 from './PaymentsAvatar1';
import styles from './PaymentsRating.module.css';

const PaymentsRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface PaymentsRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function PaymentsRating({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = PaymentsRatingSchema.safeParse({ value: val });
