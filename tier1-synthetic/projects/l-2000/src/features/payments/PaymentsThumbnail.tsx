...
import PaymentsBridge1 from './PaymentsBridge1';
import PaymentsSpinner from './PaymentsSpinner';
import styles from './PaymentsThumbnail.module.css';

interface PaymentsThumbnailItem {
  description: string;
  title: string;
  tags: string[];
  priority: number;
...
interface PaymentsThumbnailProps {
  items?: PaymentsThumbnailItem[];
...
export default function PaymentsThumbnail({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: PaymentsThumbnailProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'title', 'tags']);
