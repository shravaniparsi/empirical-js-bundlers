...
import styles from './PaymentsAvatar2.module.css';

interface PaymentsAvatar2Item {
  title: string;
  assignee: string;
  createdAt: Date;
  description: string;
...
interface PaymentsAvatar2Props {
  items?: PaymentsAvatar2Item[];
...
export default function PaymentsAvatar2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: PaymentsAvatar2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'assignee', 'createdAt']);
