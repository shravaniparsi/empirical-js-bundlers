...
import PaymentsPaginated from './PaymentsPaginated';
import PaymentsBridge1 from './PaymentsBridge1';
import CatalogPieChart from '../catalog/CatalogPieChart';
import styles from './PaymentsInput1.module.css';

const PaymentsInput1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface PaymentsInput1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function PaymentsInput1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = PaymentsInput1Schema.safeParse({ value: val });
