...
import { z } from 'zod';
import PaymentsScore from './PaymentsScore';
import styles from './PaymentsColorPicker.module.css';

const PaymentsColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface PaymentsColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function PaymentsColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = PaymentsColorPickerSchema.safeParse({ value: val });
