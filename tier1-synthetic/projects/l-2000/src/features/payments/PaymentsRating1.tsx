...
import { z } from 'zod';
import PaymentsInput1 from './PaymentsInput1';
import styles from './PaymentsRating1.module.css';

const PaymentsRating1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface PaymentsRating1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function PaymentsRating1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = PaymentsRating1Schema.safeParse({ value: val });
