...
import { z } from 'zod';
import PaymentsSpinner1 from './PaymentsSpinner1';
import styles from './PaymentsFileUpload.module.css';

const PaymentsFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface PaymentsFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function PaymentsFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = PaymentsFileUploadSchema.safeParse({ value: val });
