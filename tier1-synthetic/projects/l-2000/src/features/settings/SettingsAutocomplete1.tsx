...
import styles from './SettingsAutocomplete1.module.css';

const SettingsAutocomplete1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SettingsAutocomplete1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SettingsAutocomplete1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SettingsAutocomplete1Schema.safeParse({ value: val });
