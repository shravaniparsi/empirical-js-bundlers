...
import styles from './SettingsRating.module.css';

const SettingsRatingSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SettingsRatingProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SettingsRating({
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SettingsRatingSchema.safeParse({ value: val });
