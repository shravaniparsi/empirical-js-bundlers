...
import ReviewsFileUpload from './ReviewsFileUpload';
import ReviewsConfirm from './ReviewsConfirm';
import styles from './ReviewsSummary2.module.css';

interface ReviewsSummary2Item {
  assignee: string;
  updatedAt: Date;
  title: string;
  status: string;
...
interface ReviewsSummary2Props {
  items?: ReviewsSummary2Item[];
...
export default function ReviewsSummary2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: ReviewsSummary2Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'updatedAt', 'title']);
