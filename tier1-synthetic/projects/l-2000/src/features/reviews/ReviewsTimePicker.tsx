...
import ReviewsScore from './ReviewsScore';
import ReviewsDrawer from './ReviewsDrawer';
import DocumentsDonut from '../documents/DocumentsDonut';
import CatalogPieChart from '../catalog/CatalogPieChart';
import NotificationsMeter from '../notifications/NotificationsMeter';
import styles from './ReviewsTimePicker.module.css';

const ReviewsTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsTimePickerSchema.safeParse({ value: val });
