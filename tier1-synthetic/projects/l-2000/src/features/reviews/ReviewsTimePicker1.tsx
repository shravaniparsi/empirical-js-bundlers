...
import { z } from 'zod';
import ReviewsThumbnail1 from './ReviewsThumbnail1';
import DocumentsScatter from '../documents/DocumentsScatter';
import styles from './ReviewsTimePicker1.module.css';

const ReviewsTimePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsTimePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsTimePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsTimePicker1Schema.safeParse({ value: val });
