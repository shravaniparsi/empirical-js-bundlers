...
import { z } from 'zod';
import ReviewsRank1 from './ReviewsRank1';
import CheckoutScroll from '../checkout/CheckoutScroll';
import styles from './ReviewsRadio1.module.css';

const ReviewsRadio1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ReviewsRadio1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ReviewsRadio1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ReviewsRadio1Schema.safeParse({ value: val });
