...
import styles from './ReviewsDetail4.module.css';

interface ReviewsDetail4Item {
  description: string;
  title: string;
  createdAt: Date;
  updatedAt: Date;
...
interface ReviewsDetail4Props {
  items?: ReviewsDetail4Item[];
...
export default function ReviewsDetail4({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'title', 'createdAt']);
