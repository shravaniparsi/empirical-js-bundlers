...
import ReviewsSummary2 from './ReviewsSummary2';
import ProductsComposer from '../products/ProductsComposer';
import CheckoutScroll from '../checkout/CheckoutScroll';
import styles from './ReviewsThumbnail.module.css';

interface ReviewsThumbnailItem {
  status: string;
  updatedAt: Date;
  assignee: string;
  id: string;
...
interface ReviewsThumbnailProps {
  items?: ReviewsThumbnailItem[];
...
export default function ReviewsThumbnail({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: ReviewsThumbnailProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'updatedAt', 'assignee']);
