...
import CategoriesSuspense2 from './CategoriesSuspense2';
import CategoriesSuspense from './CategoriesSuspense';
import styles from './CategoriesTag1.module.css';

interface CategoriesTag1Item {
  description: string;
  createdAt: Date;
  updatedAt: Date;
  title: string;
...
interface CategoriesTag1Props {
  items?: CategoriesTag1Item[];
...
export default function CategoriesTag1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['description', 'createdAt', 'updatedAt']);
