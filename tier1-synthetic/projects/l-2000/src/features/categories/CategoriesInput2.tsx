...
import styles from './CategoriesInput2.module.css';

const CategoriesInput2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CategoriesInput2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CategoriesInput2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CategoriesInput2Schema.safeParse({ value: val });
