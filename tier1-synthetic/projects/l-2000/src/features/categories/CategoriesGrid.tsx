...
import CategoriesRank2 from './CategoriesRank2';
import CategoriesFileUpload from './CategoriesFileUpload';
import PaymentsContext from '../payments/PaymentsContext';
import styles from './CategoriesGrid.module.css';

interface CategoriesGridItem {
  status: string;
  tags: string[];
  createdAt: Date;
  id: string;
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'tags', 'createdAt']);
