...
import styles from './CategoriesAutocomplete2.module.css';

const CategoriesAutocomplete2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface CategoriesAutocomplete2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function CategoriesAutocomplete2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = CategoriesAutocomplete2Schema.safeParse({ value: val });
