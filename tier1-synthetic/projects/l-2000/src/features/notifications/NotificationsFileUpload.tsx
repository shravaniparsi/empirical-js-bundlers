...
import NotificationsProgress from './NotificationsProgress';
import NotificationsCheckbox from './NotificationsCheckbox';
import ProfileProvider from '../profile/ProfileProvider';
import styles from './NotificationsFileUpload.module.css';

const NotificationsFileUploadSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface NotificationsFileUploadProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function NotificationsFileUpload({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = NotificationsFileUploadSchema.safeParse({ value: val });
