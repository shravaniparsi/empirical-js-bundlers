...
import NotificationsRank1 from './NotificationsRank1';
import NotificationsMeter from './NotificationsMeter';
import NotificationsAreaChart from './NotificationsAreaChart';
import styles from './NotificationsTimeline1.module.css';

interface NotificationsTimeline1Item {
  status: string;
  priority: number;
  updatedAt: Date;
  id: string;
...
interface NotificationsTimeline1Props {
  items?: NotificationsTimeline1Item[];
...
export default function NotificationsTimeline1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: NotificationsTimeline1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['status', 'priority', 'updatedAt']);
