...
import styles from './NotificationsRangeSlider.module.css';

const NotificationsRangeSliderSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface NotificationsRangeSliderProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function NotificationsRangeSlider({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = NotificationsRangeSliderSchema.safeParse({ value: val });
