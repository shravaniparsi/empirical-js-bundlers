...
import NotificationsStat from './NotificationsStat';
import ProfileFunnel from '../profile/ProfileFunnel';
import styles from './NotificationsInput1.module.css';

const NotificationsInput1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface NotificationsInput1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function NotificationsInput1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = NotificationsInput1Schema.safeParse({ value: val });
