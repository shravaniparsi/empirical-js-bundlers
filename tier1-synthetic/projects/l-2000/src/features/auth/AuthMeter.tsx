...
import AuthFeed1 from './AuthFeed1';
import AuthAreaChart from './AuthAreaChart';
import DocumentsScroll from '../documents/DocumentsScroll';
import styles from './AuthMeter.module.css';

interface AuthMeterItem {
  priority: number;
  tags: string[];
  createdAt: Date;
  title: string;
...
export default function AuthMeter({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['priority', 'tags', 'createdAt']);
