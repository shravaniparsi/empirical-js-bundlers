...
import styles from './AuthDatePicker1.module.css';

const AuthDatePicker1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AuthDatePicker1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AuthDatePicker1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AuthDatePicker1Schema.safeParse({ value: val });
