...
import styles from './AuthToggle1.module.css';

const AuthToggle1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface AuthToggle1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function AuthToggle1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = AuthToggle1Schema.safeParse({ value: val });
