...
import styles from './ProductsRangeSlider1.module.css';

const ProductsRangeSlider1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProductsRangeSlider1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProductsRangeSlider1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProductsRangeSlider1Schema.safeParse({ value: val });
