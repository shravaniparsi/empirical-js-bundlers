...
import { z } from 'zod';
import ProductsTextArea from './ProductsTextArea';
import ProductsNotification from './ProductsNotification';
import ProductsSkeleton from './ProductsSkeleton';
import styles from './ProductsRadio1.module.css';

const ProductsRadio1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface ProductsRadio1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function ProductsRadio1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = ProductsRadio1Schema.safeParse({ value: val });
