...
import TagsBridge from './TagsBridge';
import PaymentsPieChart2 from '../payments/PaymentsPieChart2';
import styles from './TagsDetail1.module.css';

interface TagsDetail1Item {
  createdAt: Date;
  priority: number;
  assignee: string;
  id: string;
...
interface TagsDetail1Props {
  items?: TagsDetail1Item[];
...
export default function TagsDetail1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'priority', 'assignee']);
