...
import styles from './IntegrationsTimePicker.module.css';

const IntegrationsTimePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface IntegrationsTimePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function IntegrationsTimePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = IntegrationsTimePickerSchema.safeParse({ value: val });
