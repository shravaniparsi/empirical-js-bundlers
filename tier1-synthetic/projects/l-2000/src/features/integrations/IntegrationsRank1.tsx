...
import IntegrationsResponsive from './IntegrationsResponsive';
import IntegrationsSummary from './IntegrationsSummary';
import IntegrationsHeatmap from './IntegrationsHeatmap';
import styles from './IntegrationsRank1.module.css';

interface IntegrationsRank1Item {
  createdAt: Date;
  updatedAt: Date;
  assignee: string;
  id: string;
...
interface IntegrationsRank1Props {
  items?: IntegrationsRank1Item[];
...
export default function IntegrationsRank1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: IntegrationsRank1Props) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['createdAt', 'updatedAt', 'assignee']);
