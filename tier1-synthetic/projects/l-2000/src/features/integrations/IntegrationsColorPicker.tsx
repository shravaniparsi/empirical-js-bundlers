...
import styles from './IntegrationsColorPicker.module.css';

const IntegrationsColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface IntegrationsColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function IntegrationsColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = IntegrationsColorPickerSchema.safeParse({ value: val });
