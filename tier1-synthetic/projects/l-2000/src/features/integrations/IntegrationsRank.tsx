...
import styles from './IntegrationsRank.module.css';

interface IntegrationsRankItem {
  assignee: string;
  createdAt: Date;
  description: string;
  updatedAt: Date;
...
interface IntegrationsRankProps {
  items?: IntegrationsRankItem[];
...
export default function IntegrationsRank({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
  maxItems = 10,
}: IntegrationsRankProps) {
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['assignee', 'createdAt', 'description']);
