...
import styles from './IntegrationsTextArea1.module.css';

const IntegrationsTextArea1Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface IntegrationsTextArea1Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function IntegrationsTextArea1({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = IntegrationsTextArea1Schema.safeParse({ value: val });
