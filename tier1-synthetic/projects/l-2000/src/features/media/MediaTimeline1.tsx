...
import MediaSplit1 from './MediaSplit1';
import styles from './MediaTimeline1.module.css';

interface MediaTimeline1Item {
  tags: string[];
  priority: number;
  title: string;
  id: string;
...
interface MediaTimeline1Props {
  items?: MediaTimeline1Item[];
...
export default function MediaTimeline1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['tags', 'priority', 'title']);
