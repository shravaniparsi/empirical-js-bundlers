...
import styles from './MediaDatePicker.module.css';

const MediaDatePickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface MediaDatePickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function MediaDatePicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = MediaDatePickerSchema.safeParse({ value: val });
