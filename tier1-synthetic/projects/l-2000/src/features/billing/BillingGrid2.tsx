...
import BillingInfiniteScroll from './BillingInfiniteScroll';
import BillingHeatmap from './BillingHeatmap';
import BillingList2 from './BillingList2';
import styles from './BillingGrid2.module.css';

interface BillingGrid2Item {
  title: string;
  updatedAt: Date;
  createdAt: Date;
  id: string;
...
interface BillingGrid2Props {
  items?: BillingGrid2Item[];
...
export default function BillingGrid2({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['title', 'updatedAt', 'createdAt']);
