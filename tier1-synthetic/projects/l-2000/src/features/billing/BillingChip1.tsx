...
import BillingResponsive1 from './BillingResponsive1';
import WebhooksRegistry from '../webhooks/WebhooksRegistry';
import styles from './BillingChip1.module.css';

interface BillingChip1Item {
  updatedAt: Date;
  description: string;
  assignee: string;
  priority: number;
...
interface BillingChip1Props {
  items?: BillingChip1Item[];
...
export default function BillingChip1({
  items = [],
  onItemClick,
  variant = 'detailed',
  className,
    ...
    <div className={clsx(styles.container, styles[variant], className)}>
      ...
      <ul className={styles.list}>
        {visibleItems.map(item => {
          const picked = pick(item, ['updatedAt', 'description', 'assignee']);
