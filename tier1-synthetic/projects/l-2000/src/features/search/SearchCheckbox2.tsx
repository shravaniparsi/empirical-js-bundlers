...
import { z } from 'zod';
import SearchProvider1 from './SearchProvider1';
import styles from './SearchCheckbox2.module.css';

const SearchCheckbox2Schema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SearchCheckbox2Props {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SearchCheckbox2({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SearchCheckbox2Schema.safeParse({ value: val });
