...
import styles from './SearchColorPicker.module.css';

const SearchColorPickerSchema = z.object({
  value: z.string().min(1, 'This field is required').max(255, 'Too long'),
});

interface SearchColorPickerProps {
  label: string;
  name: string;
  value?: string;
  defaultValue?: string;
...
export default function SearchColorPicker({
  label,
  name,
  ...
  const debouncedValidate = useCallback(
    debounce((val: string) => {
      const result = SearchColorPickerSchema.safeParse({ value: val });
